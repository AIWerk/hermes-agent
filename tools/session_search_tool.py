...
def _discover_payload(db, query: str, detail: str, results: list, **extra) -> str:
    ...
    health = _quiet(db.fts_health_state, None, "fts_health_state failed")
    if isinstance(health, dict) and health.get("repair_required"):
        extra.setdefault(
            "search_provenance",
            {"degraded": True, "engine": "canonical_like", "repair_required": True},
        )
        extra.setdefault(
            "operator_alert",
            {
                "severity": health.get("severity", "error"),
