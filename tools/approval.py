...
def _is_interactive_cli() -> bool:
    ...
    try:
        from tools import approval_context
        return approval_context._is_interactive_cli()
    except Exception:
        pass
...
def get_current_session_key(default: str = "default") -> str:
    ...
    Resolution order:
    1. split approval_context contextvars (set by gateway before agent.run)
    2. facade-local legacy contextvars
    3. session_context / os.environ fallback
    """
    split_key = approval_context.get_current_session_key(default="")
    if split_key:
        return split_key
...
def check_execute_code_guard(code: str, env_type: str, has_host_access: bool = False) -> dict:
    ...
    if not is_cli and not is_gateway and not is_ask:
        for ctx in _unattended_contexts():
            if ctx.mode() == "deny":
                return {
                    "approved": False,
                    "message": f"BLOCKED: execute_code runs arbitrary local Python. {ctx.exec_tail}",
                    "pattern_key": pattern_key,
    ...
    if _is_single_query_approval_context():
        ...
        if _approval_context._get_single_query_approval_mode() != "approve":
            return {
                "approved": False,
                "message": (
...
from tools.approval_context import (  # noqa: E402,F811
...
from tools.approval_detection import (  # noqa: E402,F811
    DANGEROUS_PATTERNS,
    DANGEROUS_PATTERNS_COMPILED,
    HARDLINE_PATTERNS,
    HARDLINE_PATTERNS_COMPILED,
