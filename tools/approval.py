...
def _is_cui_managed_autonomy_enabled() -> bool:
    ...
    """
    # The launch-time flag is stable for the process; per-turn actor identity is
    # isolated in the ContextVar. Both are required.
...
def check_all_command_guards(command: str, env_type: str,
    ...
    # User-defined deny rules (approvals.deny in config.yaml): like the
    # hardline floor, these fire BEFORE scoped autonomy / yolo / mode=off — a
    # deny rule is the user saying "never".
    deny_pattern = _match_user_deny_rule(command)
    if deny_pattern is not None:
        logger.warning("User deny rule %r blocked command: %s",
                       deny_pattern, command[:200])
    ...
    # prompts for lay users while preserving the hardline, sudo, and user deny
    # floors above. This is narrower than global YOLO because the dashboard must
    # inject both the flag and trusted tenant actor context into assistant-mode.
