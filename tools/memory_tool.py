...
def _memory_router_curation_enabled() -> bool:
    ...
    try:
        ...
        config = load_config()
        return bool(cfg_get(config, "memory", "router", "enabled", default=True)) and bool(
            cfg_get(config, "memory", "router", "block_non_inject_writes", default=True)
        )
    except Exception:
        return False
...
def _memory_router_block(
    ...
    *,
    ...
    allowed, route = should_write_builtin_memory(content or "", target=target, metadata=metadata)
    ...
    *,
    ...
    for operation in operations:
        if not isinstance(operation, dict):
            return {"success": False, "error": "Each operation must be an object."}
...
def memory_tool(action: str = None, target: str = "memory", content: str = None, old_text: str = None,
    ...
    from agent.cui_actor_context import cui_admin_memory_block_result, memory_write_blocked_for_cui_admin
    ...
    if invalid is None and action in {"add", "replace"}:
        blocked = _memory_router_block(content or "", target)
        if blocked is not None:
            invalid = json.dumps(blocked, ensure_ascii=False)
    if invalid is None:
        invalid = _apply_write_gate(action, target, content, old_text)
...
def apply_memory_pending(payload: Dict[str, Any], store: "MemoryStore") -> Dict[str, Any]:
    ...
    actor_guard_args = {"action": action, "operations": payload.get("operations")}
    if memory_write_blocked_for_cui_admin("memory", actor_guard_args):
    ...
    if action == "batch":
        ...
        blocked = _memory_router_block_batch(target, operations, metadata=payload.get("metadata"))
        if blocked is not None:
            return blocked
    ...
    if action in {"add", "replace"}:
        blocked = _memory_router_block(
            payload.get("content") or payload.get("new_text") or "",
            target,
