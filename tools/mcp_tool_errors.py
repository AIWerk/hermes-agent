...
def _make_redirect_header_stripper(original_url, *, strict: bool = False,
    ...
    """httpx request/response hook stripping sensitive headers off-origin.

    Request hooks are the authoritative path: httpx has built the redirect request by then but
    has not sent it. Response-hook support remains for compatibility with direct callers/tests.
    """
    origin = (original_url.scheme, original_url.host, original_url.port)

    async def _strip_on_cross_origin_redirect(message):
        if hasattr(message, "next_request"):
            next_request = message.next_request if message.is_redirect else None
            target = next_request.url if next_request is not None else None
            headers = next_request.headers if next_request is not None else None
        else:
            target = message.url
