...
def _open_child_session_db(parent_agent) -> Any:
    ...
    with _quiet("subagent: failed to open dedicated SessionDB; child persistence disabled", exc_info=True):
        ...
        if _parent_db_path is not None and not isinstance(_parent_db_path, (str, Path)):
            return None
        return acquire(Path(_parent_db_path)) if _parent_db_path is not None else acquire()
    return None
