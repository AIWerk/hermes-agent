...
class MCPServerTask(MCPServerRunMixin, MCPServerTransportMixin, MCPServerHealthMixin):
    ...
    __slots__ = (
        "name", "state_key", "registry_scope", "session", "tool_timeout", "_task", "_ready", "_shutdown_event", "_reconnect_event",
        ...
        "_death_supervisor_reservation",
        "_ever_connected")

    def __init__(self, name: str, registry_scope: Optional[str] = None):
        self.name = name
        self.registry_scope = registry_scope
        self.state_key = _server_state_key(name, registry_scope)
        ...
        self._death_supervisor_reservation: Optional[str] = None
        self._auth_type: str = ""
...
_SERVER_SCOPE_CURRENT = object()
_server_state_key_override: contextvars.ContextVar[Optional[tuple[str, Any]]] = (
    contextvars.ContextVar("mcp_server_state_key_override", default=None)
)
...
_server_connecting: set[Any] = set()
...
# Claimed tasks exist before adoption into _servers. Teardown removes their
# authority so an in-flight connect cannot resurrect a removed configuration.
_server_connect_claims: Dict[Any, MCPServerTask] = {}
...
_server_connect_retry_after: Dict[Any, float] = {}   # scoped name -> monotonic deadline
_server_connect_failures: Dict[Any, int] = {}        # scoped name -> consecutive failures
...
def _bump_server_error(server_name: str, *, state_key=None) -> None:
    """Count a failure; at the threshold (re)stamp the breaker-open time."""
    key = state_key if state_key is not None else _server_state_key(server_name)
...
def _reset_server_error(server_name: str, *, state_key=None) -> None:
...
_parallel_safe_servers: set[Any] = set()
# Scoped registry tool key -> scoped raw server key (generated names are lossy; never re-parse).
_mcp_tool_server_names: Dict[Any, Any] = {}
...
# The SDK exposes streams, not its subprocess handle. Serialize only the
# snapshot -> spawn -> PID-claim window so concurrent stdio starts cannot
# claim each other's children; sessions remain fully concurrent afterward.
...
_death_supervisor_lock = threading.RLock()
...
_supervised_pgid_identities: dict[int, "_DeathSupervisorIdentity"] = {}
_pending_death_supervisor_reservations: set = set()
_SUPERVISOR_READY_TIMEOUT_S = 5.0


@dataclass(frozen=True)
class _DeathSupervisorIdentity:
    pid: int
    pgid: int
    start_time: str | None
...
def _death_supervisor_register_payload() -> str:
    lines = []
    for pgid in _supervised_pgids:
        identity = _supervised_pgid_identities.get(pgid)
        if identity is None:
            lines.append(f"register {pgid}\n")
        else:
            lines.append(f"register {identity.pid} {identity.pgid} {identity.start_time or 'unknown'}\n")
    return "".join(lines)


def _cleanup_death_supervisor_process(proc) -> None:
    ...
    if stdout is not None:
        try:
            ...
            pass
    socket_path = getattr(proc, "_hermes_socket_path", None)
    if socket_path:
        try:
            os.unlink(socket_path)
        except OSError:
            pass
    socket_dir = getattr(proc, "_hermes_socket_dir", None)
    if socket_dir:
        try:
            os.rmdir(socket_dir)
        except OSError:
...
def _wait_for_death_supervisor_ready(proc) -> bool:
    import selectors
    stdout = getattr(proc, "stdout", None)
    if stdout is None:
        return False
    ...
    try:
        ...
        events = sel.select(_SUPERVISOR_READY_TIMEOUT_S)
        if not events:
            return False
        return stdout.readline().strip() == "ready"
    finally:
        sel.close()
        try:
            stdout.close()
...
def _spawn_death_supervisor():
    ...
    supervisor = os.path.join(tools_dir, "mcp_death_supervisor.py")
    socket_dir = tempfile.mkdtemp(prefix="hermes-mcp-supervisor-")
    ...
    try:
        ...
        proc = subprocess.Popen(
            ...
            stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=_get_mcp_stderr_log(),
            start_new_session=True, close_fds=True, text=True)
        proc._hermes_socket_path = socket_path
        proc._hermes_socket_dir = socket_dir
        if not _wait_for_death_supervisor_ready(proc):
            try:
                proc.kill()
                proc.wait(timeout=5)
            except Exception:
                pass
            _cleanup_death_supervisor_process(proc)
            return None
        return proc
    except Exception:
...
def _ensure_death_supervisor_socket() -> str:
    """Start/reuse the shared supervisor and return its acknowledged-registration socket."""
    if os.name != "posix":
        raise RuntimeError("MCP death supervisor is POSIX-only")
    ...
    with _death_supervisor_lock:
        ...
        if proc is None:
            ...
            if proc is None:
                raise RuntimeError("could not start MCP parent-death supervisor")
            payload = _death_supervisor_register_payload()
            payload += "".join(f"reserve {token}\n" for token in _pending_death_supervisor_reservations)
            if payload:
                try:
                    ...
                    _cleanup_death_supervisor_process(proc)
                    _death_supervisor = None
                    raise RuntimeError("could not replay MCP death-supervisor registrations") from exc
        elif stale:
            ...
            try:
                ...
                _death_supervisor = proc
                if proc is None:
                    raise RuntimeError("could not restart MCP parent-death supervisor after stale prune") from exc
                payload = _death_supervisor_register_payload()
                payload += "".join(f"reserve {token}\n" for token in _pending_death_supervisor_reservations)
                try:
                    proc.stdin.write(payload)
                    proc.stdin.flush()
                except (BrokenPipeError, ValueError, OSError) as replay_exc:
                    _cleanup_death_supervisor_process(proc)
                    _death_supervisor = None
                    raise RuntimeError("could not replay MCP death-supervisor registrations") from replay_exc
        socket_path = getattr(proc, "_hermes_socket_path", None)
        if not socket_path:
            raise RuntimeError("MCP parent-death supervisor has no registration socket")
        return socket_path


def _reserve_death_supervisor_registration() -> tuple[str, str]:
    """Reserve one single-use bootstrap registration token and return ``(socket, token)``."""
    if os.name != "posix":
        raise RuntimeError("MCP death supervisor is POSIX-only")
    global _death_supervisor
    token = secrets.token_urlsafe(32)
    with _death_supervisor_lock:
        socket_path = _ensure_death_supervisor_socket()
        proc = _death_supervisor
        if proc is None:
            raise RuntimeError("could not start MCP parent-death supervisor")
        try:
            proc.stdin.write(f"reserve {token}\n")
            proc.stdin.flush()
        except (BrokenPipeError, ValueError, OSError) as exc:
            _cleanup_death_supervisor_process(proc)
            proc = _spawn_death_supervisor()
            _death_supervisor = proc
            if proc is None:
                raise RuntimeError("could not restart MCP parent-death supervisor") from exc
            payload = _death_supervisor_register_payload()
            payload += "".join(f"reserve {pending}\n" for pending in _pending_death_supervisor_reservations)
            payload += f"reserve {token}\n"
            try:
                proc.stdin.write(payload)
                proc.stdin.flush()
            except (BrokenPipeError, ValueError, OSError) as replay_exc:
                _cleanup_death_supervisor_process(proc)
                _death_supervisor = None
                raise RuntimeError("could not reserve MCP death-supervisor registration") from replay_exc
            socket_path = getattr(proc, "_hermes_socket_path", None)
            if not socket_path:
                raise RuntimeError("MCP parent-death supervisor has no registration socket")
        _pending_death_supervisor_reservations.add(token)
        return socket_path, token


def _adopt_death_supervisor_reservation(token: str | None) -> None:
    if not token or os.name != "posix":
        return
    with _death_supervisor_lock:
        _pending_death_supervisor_reservations.discard(token)


def _cancel_death_supervisor_reservation(token: str | None) -> None:
    if not token or os.name != "posix":
        return
    global _death_supervisor
    with _death_supervisor_lock:
        if token not in _pending_death_supervisor_reservations:
            return
        _pending_death_supervisor_reservations.discard(token)
        proc = _death_supervisor
        if proc is not None and proc.poll() is None:
            try:
                proc.stdin.write(f"cancel {token}\n")
                proc.stdin.flush()
            except (BrokenPipeError, ValueError, OSError):
                _cleanup_death_supervisor_process(proc)
                _death_supervisor = None
                if _supervised_pgids or _pending_death_supervisor_reservations:
                    replacement = _spawn_death_supervisor()
                    _death_supervisor = replacement
                    if replacement is not None:
                        replay = _death_supervisor_register_payload()
                        replay += "".join(
                            f"reserve {pending}\n"
                            for pending in _pending_death_supervisor_reservations
                        )
                        replacement_stdin = replacement.stdin
                        if replacement_stdin is None:
                            _cleanup_death_supervisor_process(replacement)
                            _death_supervisor = None
                        else:
                            try:
                                replacement_stdin.write(replay)
                                replacement_stdin.flush()
                            except (BrokenPipeError, ValueError, OSError):
                                _cleanup_death_supervisor_process(replacement)
                                _death_supervisor = None
        _release_idle_death_supervisor()


def _release_idle_death_supervisor() -> None:
    """Drop a pre-spawn supervisor that has no parent-side registrations."""
    if os.name != "posix":
        return
    global _death_supervisor
    with _death_supervisor_lock:
...
def _prune_dead_supervised_pgids() -> set:
    ...
    ``_death_supervisor_lock``. Identity-bearing registrations are checked by pid + pgid +
    start-time before any pgid probe, so a recycled identity is forgotten without touching the
    ...
    for pgid in list(_supervised_pgids):
        ...
        if identity is not None:
            try:
                ...
                # The group leader may exit while descendants retain the PGID. Such a PGID
                # cannot be reused until the surviving group is empty, so probe the group
                # before dropping its parent-side coverage.
                try:
                    killpg(pgid, 0)
                except ProcessLookupError:
                    stale.add(pgid)
                    ...
                    # Ambiguous/existing: preserve coverage rather than orphan a live group.
                    continue
                continue
            except (PermissionError, OSError):
                pass
    ...
    for pgid in stale:
        _supervised_pgid_identities.pop(pgid, None)
    return stale


def _update_death_supervisor(verb: str, pgids, identities: dict[int, tuple[int, str | None]] | None = None) -> None:
    ...
    with _death_supervisor_lock:
        if verb == "register":
            ...
            if identities:
                for pgid, (pid, start_time) in identities.items():
                    if pgid in wanted:
                        _supervised_pgid_identities[pgid] = _DeathSupervisorIdentity(
        ...
        if proc is None or proc.poll() is not None:
            if proc is not None:
                _cleanup_death_supervisor_process(proc)
            if not _supervised_pgids and not _pending_death_supervisor_reservations:
            ...
            if verb == "register":
                ...
                for pgid in wanted:
                    identity = _supervised_pgid_identities.get(pgid)
                    if identity is None:
                        payload += f"register {pgid}\n"
                    else:
                        payload += f"register {identity.pid} {identity.pgid} {identity.start_time or 'unknown'}\n"
            else:
        ...
        try:
            ...
            # It exited between poll() and write(). Restore coverage immediately from the
            # authoritative in-memory sets: waiting for a later lifecycle event leaves a
            # parent-death window in which every surviving group is unsupervised.
            _cleanup_death_supervisor_process(proc)
            _death_supervisor = None
            if _supervised_pgids or _pending_death_supervisor_reservations:
                replacement = _spawn_death_supervisor()
...
def _server_state_key(name: str, scope: Any = _SERVER_SCOPE_CURRENT):
    """Internal MCP state key; multiplex profiles may reuse the same server name safely."""
    override = _server_state_key_override.get()
    if scope is _SERVER_SCOPE_CURRENT and override is not None and override[0] == name:
        return override[1]
    resolved_scope = _mcp_registry_scope() if scope is _SERVER_SCOPE_CURRENT else scope
    return name if resolved_scope is None else (resolved_scope, name)


def _server_name_from_state_key(key: Any) -> str:
    return str(key[1]) if isinstance(key, tuple) and len(key) == 2 else str(key)


def _server_scope_from_state_key(key: Any) -> Optional[str]:
    return str(key[0]) if isinstance(key, tuple) and len(key) == 2 else None
