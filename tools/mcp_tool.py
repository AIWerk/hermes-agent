...
class MCPServerTask:
    ...
    async def _wait_for_lifecycle_event(self) -> str:
        ...
        The cadence is ``keepalive_interval`` from server config, with legacy
        ``keepalive_interval_seconds`` accepted as a fallback. Set either to
        0/false to disable idle keepalives and rely on reconnect-on-use.
        """
        raw_keepalive = self._config.get(
            "keepalive_interval",
            self._config.get("keepalive_interval_seconds", _DEFAULT_KEEPALIVE_INTERVAL),
        )
        try:
        ...
        if raw_keepalive is False or keepalive_interval <= 0:
            ...
            keepalive_interval = max(_MIN_KEEPALIVE_INTERVAL, keepalive_interval)
        ...
        try:
            while True:
                ...
                # to detect stale or expired sessions. Prefer ping with a
                # list_tools fallback handled by _keepalive_probe().
                if self.session:
                    try:
