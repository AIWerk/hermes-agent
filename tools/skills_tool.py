...
from tools.skills_tool_setup import (  # noqa: F401
    ...
    _ENV_VAR_NAME_RE, _get_required_environment_variables, _is_env_var_persisted,
...
_LOOKUP_HINT = "Use skills_list first to discover the exact skill name, then retry skill_view."
...
def _resolve_plugin_skill(name, file_path, task_id, preprocess):
    ...
    if plugin_skill_md is None and bare:
        plugin_skill_md = pm.find_plugin_skill(f"{namespace}:{bare}")
    ...
    if plugin_skill_md is None:
        ...
        return None, (f"{namespace}/{bare}" if bare else None)
    ...
    try:
        ...
        parsed_frontmatter = parsed[0] if isinstance(parsed, tuple) else parsed
