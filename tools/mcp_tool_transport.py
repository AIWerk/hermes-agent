...
_BOOTSTRAP_SOCKET_ENV = "HERMES_MCP_DEATH_SUPERVISOR_SOCKET"
_BOOTSTRAP_TOKEN_ENV = "HERMES_MCP_DEATH_SUPERVISOR_TOKEN"
...
@asynccontextmanager
async def _stdio_spawn_claim_window():
    """Cancellation-safe async acquisition of the process-wide spawn lock."""
    while not _core._stdio_spawn_lock.acquire(blocking=False):
        await asyncio.sleep(0.01)
    try:
        yield
    finally:
        _core._stdio_spawn_lock.release()
...
class MCPServerTransportMixin:
    ...
    def _supervised_stdio_params(self, command: str, args: list, env: dict | None):
        """Wrap POSIX stdio spawn in a bootstrap that execs only after supervisor ACK."""
        if os.name != "posix":
            return command, args, env
        socket_path, token = _core._reserve_death_supervisor_registration()
        bootstrap = os.path.join(os.path.dirname(os.path.abspath(__file__)), "mcp_stdio_bootstrap.py")
        supervised_env = dict(env or {})
        supervised_env[_BOOTSTRAP_SOCKET_ENV] = socket_path
        supervised_env[_BOOTSTRAP_TOKEN_ENV] = token
        self._death_supervisor_reservation = token
        return sys.executable, [bootstrap, command, *args], supervised_env
    ...
    def _track_spawned_children(self, new_pids: Set[int]) -> None:
        ...
        for pid in new_pids:
            try:
                pgid = os.getpgid(pid)
                new_pgids[pid] = pgid
                identities[pgid] = (pid, _core._process_start_time(pid))
                ...
                pass
        state_key = getattr(self, "state_key", self.name)  # type: ignore[attr-defined]
        with _core._lock:
            _stdio_pids.update(dict.fromkeys(new_pids, state_key))
        ...
        _core._update_death_supervisor("register", new_pgids.values(), identities=identities)
        _core._adopt_death_supervisor_reservation(getattr(self, "_death_supervisor_reservation", None))
        self._death_supervisor_reservation = None
        ...
        with _core._lock:
            for pid in new_pids:
                ...
                if _pid_exists(pid) or _pgroup_alive(_stdio_pgids.get(pid)):
                    ...
                    _orphan_stdio_pid_servers[pid] = state_key
    ...
    async def _run_stdio(self, config: dict):
        ...
        command, args, safe_env = self._supervised_stdio_params(command, args, safe_env)
        new_pids: set = set()
        try:
            ...
            async with AsyncExitStack() as transport_stack:
                # The SDK does not expose its subprocess handle. Keep this narrow process-wide
                # window exclusive so another profile/server cannot spawn between our snapshots.
                async with _stdio_spawn_claim_window():
                    # Reap orphans of prior attempts first (else retries pile up zombie pairs);
                    # off-loop because the reaper blocks up to 2s.
                    await asyncio.to_thread(_lifecycle._kill_orphaned_mcp_children)
                    pids_before = _lifecycle._snapshot_child_pids()
                    # Subprocess stderr goes to the active profile's MCP log.
                    _config._write_stderr_log_header(self.name)  # type: ignore[attr-defined]
                    errlog = _config._get_mcp_stderr_log()
                    read_stream, write_stream = await transport_stack.enter_async_context(
                        _core.stdio_client(server_params, errlog=errlog)
                    )
                    # Minus non-MCP children (slash_worker, LSP) racing into the window.
                    new_pids = _filter_mcp_children(
                        _lifecycle._snapshot_child_pids() - pids_before
                    )
                    if new_pids:
            ...
            _core._cancel_death_supervisor_reservation(getattr(self, "_death_supervisor_reservation", None))
            self._death_supervisor_reservation = None
            if not new_pids:
                _core._release_idle_death_supervisor()
    ...
    async def _preflight_content_type(self, url: str, *, headers: Optional[dict] = None,
        ...
        strip_redirect_headers = _make_redirect_header_stripper(
            _httpx.URL(url),
        ...
        try:
            async with _httpx.AsyncClient(
                ...
                event_hooks={"request": [strip_redirect_headers]},
    ...
    def _register_discovered_tools_if_needed(self) -> None:
        ...
        if self._registered_tool_names:
            return
        key = self.state_key  # type: ignore[attr-defined]
        with _core._lock:
            owned = _core._servers.get(key) is self
