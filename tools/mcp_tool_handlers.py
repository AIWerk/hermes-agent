...
def _lookup_reconnectable_server(server_name: str, require_loop: bool = False):
    ...
    with _core._lock:
        srv = _core._servers.get(_core._server_state_key(server_name))
...
def _make_tool_handler(
    server_name: str, tool_name: str, tool_timeout: float, *, state_key=None
):
    """Sync registry handler (``handler(args_dict, **kwargs) -> str``) calling an MCP tool via the background loop."""
    op = f"tools/call {tool_name}"
    captured_key = state_key if state_key is not None else _core._server_state_key(server_name)

    def _handler_scoped(args: dict, **kwargs) -> str:
    ...
    def _handler(args: dict, **kwargs) -> str:
        token = _core._server_state_key_override.set((server_name, captured_key))
        try:
            return _handler_scoped(args, **kwargs)
        finally:
...
def _make_utility_handler(op: str, log_label: str, rpc, render, required: Optional[str] = None):
    ...
    def _factory(server_name: str, tool_timeout: float, *, state_key=None):
        captured_key = state_key if state_key is not None else _core._server_state_key(server_name)
...
def _make_check_fn(server_name: str, *, state_key=None):
