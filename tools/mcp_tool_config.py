...
def _get_mcp_stderr_log() -> Any:
    """Profile-scoped append-mode handle for MCP subprocess stderr. Must expose a
    ...
    with _mcp_stderr_log_lock:
        if scope not in _mcp_stderr_log_fhs:
            try:
                log_dir = home / "logs"
                ...
                try:
                    _mcp_stderr_log_fhs[scope] = open(os.devnull, "w", encoding="utf-8")
                except Exception:
                    _mcp_stderr_log_fhs[scope] = sys.stderr
        _mcp_stderr_log_fh = _mcp_stderr_log_fhs[scope]  # compatibility/introspection only
        return _mcp_stderr_log_fhs[scope]
...
# (profile scope, server name, dotted key path) entries already warned about.
_whitespace_warned: Set[Tuple[str, str, str]] = set()
...
def _warn_hidden_whitespace(server_name: str, config: dict) -> List[str]:
    ...
    for key_path in flagged:
        warning_key = (scope, server_name, key_path)
        if warning_key not in _whitespace_warned:
            _whitespace_warned.add(warning_key)
            logger.warning(
