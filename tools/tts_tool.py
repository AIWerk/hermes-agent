...
def _resolve_tts_language(tts_config: Optional[Dict[str, Any]], provider: str) -> Optional[str]:
    ...
    if isinstance(provider_config, dict):
        provider_direct = provider_config.get("language") or provider_config.get("language_code")
        if isinstance(provider_direct, str) and provider_direct.strip():
            return provider_direct.strip()
    return None
...
def text_to_speech_tool(
    ...
    if tts_config.get("normalize_text") is not False:
        try:
