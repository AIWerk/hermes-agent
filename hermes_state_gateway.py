                ...
                WHERE o.session_key IS NULL
                  ...
                  AND NOT EXISTS (
                      SELECT 1 FROM sessions p
                       WHERE p.id = o.parent_session_id
                         AND p.end_reason = 'side_session'
                  )
                  AND NOT EXISTS (
                      SELECT 1 FROM session_stack stack
                       WHERE stack.side_session_id = o.id
                  )
...
class SessionGatewayMixin:
    ...
    def pop_side_session(
        ...
        def _do(conn):
            ...
            if not row:
                return None
            columns = {info["name"] for info in conn.execute("PRAGMA table_info(session_stack)").fetchall()}
            if "returned_at" in columns:
                conn.execute(
                    """UPDATE session_stack
                       SET status = 'returned', returned_at = ?
                       WHERE id = ?""",
                    (now, row["id"]),
                )
            else:
                conn.execute("UPDATE session_stack SET status = 'returned' WHERE id = ?", (row["id"],))
            return dict(row)
    ...
    def adopt_orphaned_gateway_session(self, orphan_id: str, donor_id: str) -> bool:
        ...
        def _do(conn):
            donor = conn.execute(
                ...
                "origin_json, display_name, source, end_reason FROM sessions WHERE id = ?",
                (donor_id,),
            ).fetchone()
            orphan = conn.execute(
                "SELECT session_key, source, model_config, parent_session_id, end_reason "
                "FROM sessions WHERE id = ?", (orphan_id,)).fetchone()
            if (donor is None or orphan is None or not donor["session_key"] or orphan["session_key"]
                    or (donor["source"] or "") != (orphan["source"] or "")):
                return False
            orphan_cfg = {}
            try:
                orphan_cfg = json.loads(orphan["model_config"] or "{}")
            except (TypeError, json.JSONDecodeError):
                orphan_cfg = {}
            if isinstance(orphan_cfg, dict) and orphan_cfg.get("_side_from") is not None:
                return False
            if orphan["parent_session_id"] == donor_id and donor["end_reason"] == "side_session":
                return False
            if conn.execute(
                "SELECT 1 FROM session_stack WHERE side_session_id = ? LIMIT 1",
                (orphan_id,),
            ).fetchone():
                return False
