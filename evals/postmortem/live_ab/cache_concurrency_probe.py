...
def patched_stream(self, **kw):
    ...
    ftxt = f0.get("content") if isinstance(f0.get("content"), str) else "".join(
        b.get("text", "") for b in (f0.get("content") or []) if isinstance(b, dict)
    )
    import re as _re
    ...
    rec = dict(worker=int(m.group(1)) if m else None, call=len(msgs), t_start=time.time(), model=kw.get("model"),
               n_msgs=len(msgs), system_sha=sha(sysm), tools_sha=sha(kw.get("tools")), msg_shas=[sha(x) for x in msgs])
