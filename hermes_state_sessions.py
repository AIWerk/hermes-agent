...
""" + _UPSERT_KEEP_EXISTING_SQL,
            ...
            WITH RECURSIVE
              ancestors(id) AS (
                ...
                WHERE parent.end_reason = 'compression'
                  ...
                  AND NOT EXISTS (
                      SELECT 1 FROM session_stack stack WHERE stack.side_session_id = child.id
                  )
              ),
              descendants(id) AS (
                SELECT ?
    ...
    def _list_row(cls, row: sqlite3.Row) -> Dict[str, Any]:
        ...
        if "short_summary" in s:
            short_summary = s.pop("short_summary", None)
            outline_json = s.pop("summary_outline_json", None)
            topics_json = s.pop("summary_topics_json", None)
            if short_summary is not None:
                s["summary"] = short_summary
                s["outline"] = _parse_json_list(outline_json)
                s["topics"] = _parse_json_list(topics_json)
    ...
    def list_sessions_rich(
        ...
        select_head = (
            ...
            + ", ss.short_summary, ss.outline_json AS summary_outline_json, ss.topics_json AS summary_topics_json"
            + f",\n                    {_PREVIEW_COL_SQL},\n                    "
        )
        prompt_join = (
    ...
    def _session_lineage_root_to_tip(self, session_id: str) -> List[str]:
        ...
        with self._read_ctx() as conn:
            while current and current not in chain and len(chain) < 100:
                row = conn.execute(
                    "SELECT id, parent_session_id, source, model_config FROM sessions WHERE id = ?", (current,),
                ).fetchone()
                if row is None:
                ...
                if not parent_id or self._is_explicit_fork_child_row(current_row):
                    break
                parent = conn.execute(
                    "SELECT id, end_reason FROM sessions WHERE id = ?", (parent_id,)
                ).fetchone()
                if parent is None:
                    break
                if parent["end_reason"] == "side_session":
                    break
                current = parent_id
