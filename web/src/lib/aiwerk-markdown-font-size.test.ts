...
describe("AIWerk CUI markdown font-size invariant", () => {
  ...
  it("keeps the jsdom runtime harness declared in workspace dependencies", () => {
    const webPackage = JSON.parse(readRuntimeFile("package.json"));
    const rootLock = JSON.parse(readFileSync(resolve(repoRoot, "../package-lock.json"), "utf8"));

    expect(webPackage.devDependencies?.jsdom).toBe("29.1.1");
    expect(webPackage.devDependencies?.["@types/jsdom"]).toBe("28.0.3");
    expect(rootLock.packages?.["web"]?.devDependencies?.jsdom).toBe("29.1.1");
    expect(rootLock.packages?.["web"]?.devDependencies?.["@types/jsdom"]).toBe("28.0.3");
  });
