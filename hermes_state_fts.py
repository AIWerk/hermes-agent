...
from hermes_state_common import (
    FTS_CJK_STALE_KEY, FTS_REPAIR_FAILURE_COUNT_KEY, FTS_REPAIR_FIRST_ERROR_AT_KEY,
    FTS_REPAIR_LAST_ERROR_AT_KEY, FTS_REPAIR_LAST_ERROR_KEY, FTS_STALE_KEY,
    _FTS_CJK_TRIGGERS, _FTS_TRIGGERS,
)
...
class SessionFtsSetupMixin:
    ...
    def _try_runtime_fts_rebuild(self, exc: sqlite3.DatabaseError) -> bool:
        """Attempt one admitted live repair before durable fail-open.

        A corrupt FTS shadow table can reject writes/searches while canonical
        rows are intact. The first in-process corruption gets one public
        ``rebuild_fts()`` attempt; consumed, refused, or failed repair falls
        through to ``_enter_fts_fail_open`` so canonical writes/searches keep
        working from durable rows.
        """
            ...
            with self._lock:
                ...
                try:
                    ...
                    self._conn.execute(
                        ...
                        (FTS_REPAIR_FIRST_ERROR_AT_KEY, now),
                    )
                    self._conn.execute(
                        "INSERT INTO state_meta (key, value) VALUES (?, ?) "
                        "ON CONFLICT(key) DO UPDATE SET value = excluded.value",
                    ...
                    self._conn.execute(
                        "INSERT INTO state_meta (key, value) VALUES (?, ?) "
                        "ON CONFLICT(key) DO UPDATE SET value = excluded.value",
                        (FTS_REPAIR_LAST_ERROR_KEY, str(exc)[:500]),
                    )
