...
from hermes_cli.browser_connect import (
    DEFAULT_BROWSER_CDP_URL, call_local_browser_launcher, discover_local_cdp_url,
    find_free_debug_port, is_browser_debug_ready, launch_chrome_debug,
    load_local_browser_launcher_config, local_port_in_use,
    manual_chrome_debug_command, wait_for_browser_debug_ready)
...
def _launch_configured_cdp_browser(port: int):
    """Try the user's configured local browser launcher before the built-in launcher."""
    cfg = load_local_browser_launcher_config()
    if not getattr(cfg, "enabled", False):
        return None
    ok, detail = call_local_browser_launcher(cfg, "open", timeout=15.0)
    if not ok:
        if detail:
            _pr(f"   ⚠ Configured browser launcher failed: {detail}")
            _pr("   Falling back to the normal local Chromium launch path")
        return None
    _pr("   Local browser launcher accepted /open")
    if wait_for_browser_debug_ready(
        getattr(cfg, "cdp_url", f"http://127.0.0.1:{port}"),
        timeout_s=getattr(cfg, "cdp_poll_timeout_s", 20.0),
    ):
        found = discover_local_cdp_url(port, timeout=1.0) or getattr(cfg, "cdp_url", None)
        if found:
            print(f"   ✓ Chromium-family browser launched and listening on port {port}")
            return found
    return None
...
def _browser_connect(cli, cdp_url: str) -> None:
    ...
    if found:
        ...
        found = _launch_configured_cdp_browser(port) or _launch_default_cdp_browser(port)
    else:
    ...
    if hasattr(cli, '_pending_input'):
        cli._pending_input.put(
            ...
            "control that CDP browser. This is typically a Hermes-managed "
            "isolated debug profile, not the user's main everyday browser. It is still user-visible and may contain "
            "pages, logged-in sessions, or cookies in that debug profile, so avoid destructive actions, "
            "closing tabs, or navigating away unless the user's task calls for it; normal approval flow "
            "still applies.]")
...
class CLICommandsMixin:
    ...
    def _sync_agent_after_session_switch(
        ...
        *,
        ...
        if hasattr(self.agent, "session_log_file") and hasattr(self.agent, "logs_dir"):
            self.agent.session_log_file = self.agent.logs_dir / f"session_{target_session_id}.json"
    ...
    def _handle_side_command(self, cmd_original: str) -> None:
        ...
        raw_title = _command_arg(cmd_original)
        title = None
        if raw_title:
            try:
                from hermes_state import SessionDB
                ...
                return _cp(f"  Title rejected: {exc}")
        ...
        try:
            if not self._session_db.get_session(parent_session_id):
                self._session_db.create_session(session_id=parent_session_id, source=source, model=self.model)
            ...
            return _cp(f"  Failed to start side session: {exc}")
    ...
    def _handle_back_command(self, cmd_original: str) -> None:
        ...
        source = os.environ.get("HERMES_SESSION_SOURCE", "cli")
        try:
            active = self._session_db.pop_side_session(source=source, side_session_id=self.session_id)
        except Exception as exc:
            return _cp(f"  Failed to return from side session: {exc}")
        if not active:
            return

        side_session_id = active.get("side_session_id") or self.session_id
        parent_session_id = active.get("parent_session_id")
        ...
        self.session_start = now
        restored_history = self._session_db.get_messages_as_conversation(
            parent_session_id,
            repair_alternation=True,
        )
        self.conversation_history = [
            {key: value for key, value in message.items() if key not in {"_db_persisted", "timestamp"}}
    ...
    def _handle_background_command(self, cmd: str):
        ...
        from cli import _clear_cli_agent_thread_callbacks, _install_cli_agent_thread_callbacks
        import run_agent
        ...
        def produce():
            callback_scope = _install_cli_agent_thread_callbacks(self, session_id=task_id)
            try:
                bg_agent = run_agent.AIAgent(
                ...
                _clear_cli_agent_thread_callbacks(callback_scope)
    ...
    def _handle_browser_command(self, cmd: str):
        ...
        if sub == "use" or sub.startswith("use "):
            ...
            _browser_connect(self, DEFAULT_BROWSER_CDP_URL)
        elif sub == "disconnect":
