...
class CLILoopsMixin:
    ...
    def _cmd_fresh(self, cmd_original: str):
        ...
        if self._confirm_destructive_slash(
            "fresh",
            "This starts a fresh session.\n"
            "Recent messages will be carried as read-only context.",
            cmd_original=cmd_original,
        ) is None:
            return True
        ...
        _cprint(f"  Carried {carried} message(s) into the fresh session.")
        return True

    def _handle_fresh_command(self, cmd_original: str):
        return self._cmd_fresh(cmd_original)
