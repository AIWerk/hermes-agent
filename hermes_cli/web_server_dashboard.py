...
def mount_spa(application: FastAPI, *, bootstrap_js_getter=None):
    ...
    def _serve_index(prefix: str = ""):
        ...
        extra_bootstrap_js = bootstrap_js_getter() if callable(bootstrap_js_getter) else ""
        bootstrap_script = (
            f"<script>{token_js}"
            f"window.__HERMES_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
            f'window.__HERMES_BASE_PATH__="{prefix}";'
            f"window.__HERMES_AUTH_REQUIRED__={'true' if gated else 'false'};"
            f"{extra_bootstrap_js}"
            f"</script>"
        )
        if prefix:
