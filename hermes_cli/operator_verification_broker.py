...
def _request_key(raw_request: bytes) -> tuple[str, str]:
    ...
    try:
        ...
        pass
    return request_key, capability


def _serve(socket_path: Path, result: dict[str, Any], cache_key: str, capability: str, allowed_client_pid: int) -> int:
    ...
    try:
        ...
        deadline = min(time.monotonic() + _IDLE_TIMEOUT_SECONDS, time.monotonic() + max(0, int(result["expires_at"]) - int(time.time())))
            ...
            with conn:
                try:
                    ...
                    if hasattr(socket, "SO_PEERCRED"):
                        ...
                        if peer_pid != allowed_client_pid:
                            conn.sendall(b"{}\n")
                            continue
                    else:
                        conn.sendall(b"{}\n")
                        continue
                    request_key, request_capability = _request_key(conn.recv(_MAX_PAYLOAD_BYTES))
...
def main(argv: list[str] | None = None) -> int:
    ...
    if result is None:
        return 2
    return _serve(Path(argv[0]), result, cache_key, capability, parent_pid)
