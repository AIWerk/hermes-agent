...
class AuthStoreReadError(AuthError, OSError):
    """Unreadable auth.json; never degrade this to an empty/logged-out store."""

    def __init__(self, errno_value: int | None, message: str) -> None:
        OSError.__init__(self, errno_value, message)
        AuthError.__init__(self, message)


def _preserve_file_owner(path: Path) -> tuple[int, int] | None:
    """Existing uid/gid for an auth file before atomic replacement."""
    try:
        st = path.stat()
    except OSError:
        return None
...
def _restore_file_owner(path: Path, owner: tuple[int, int] | None) -> None:
    """Best-effort owner restore after atomic replacement."""
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        msg = (
            f"Could not read auth store {auth_file}; check ownership/permissions. "
            "Hermes is refusing to treat this as logged out."
        )
        raise AuthStoreReadError(exc.errno, msg) from exc
...
def _save_auth_store(auth_store: Dict[str, Any], target_path: Optional[Path] = None) -> Path:
    ...
    owner = _preserve_file_owner(auth_file)
    _write_private_file_atomic(auth_file, json.dumps(auth_store, indent=2) + "\n", fsync_dir=True)
    _restore_file_owner(auth_file, owner)
    try:
