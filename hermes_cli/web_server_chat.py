...
_CUI_MANAGED_TRUST_ENV = frozenset({
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    if auth_required:
        ...
        def _stamp_identity(info, *, ticket: bool) -> None:
            ...
            # Only allowlisted identity fields are carried — bookkeeping such as
            # ``minted_at`` and private contact fields are not part of the
            # transport identity contract.
            keys = (
                ("tenant_id", "actor_id", "role", "display_name", "user_id", "provider")
                if ticket else ("user_id", "provider")
            )
            ws._hermes_auth_identity = {
                key: info.get(key) for key in keys if info.get(key)
            }

        internal = ws.query_params.get("internal", "")
        if internal:
            try:
                _stamp_identity(consume_internal_credential(internal), ticket=False)
        ...
        try:
            _stamp_identity(consume_ticket(ticket), ticket=True)
            if protocol_ticket:
...
def _resolve_chat_argv(
    ...
    authenticated_actor: Optional[dict] = None,
    ...
    for key in _CUI_MANAGED_TRUST_ENV:
        env.pop(key, None)
