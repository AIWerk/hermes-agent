...
def _cmd_repair_search(args):
    ...
    if report.get("verified") and report.get("reason") == "not-required":
        print("✓ Search-index repair not required; repair-required is already clear.")
        return 0
    if not report.get("verified") or not report.get("repaired"):
        reason = report.get("reason", "verification-failed")
        print(f"✗ Search-index repair was not verified; repair-required remains set ({reason}).")
        return 2
    print("✓ Verified offline search-index repair completed; repair-required was cleared.")
...
_PRE_DB_HANDLERS = {
    "repair": _cmd_repair,
    "repair-search": _cmd_repair_search,
