...
def _qualified_release_marker(project_root: Path) -> Path:
    return project_root / "qualified-release.json"
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    project_root = _web_project_root(web_dir)
    dist_dir = _web_dist_dir(web_dir)
    if _qualified_release_marker(project_root).is_file():
        return any(p.exists() for p in (dist_dir / ".vite" / "manifest.json", dist_dir / "index.html"))
    ...
    try:
        ...
        return _do_build_web_ui_via_facade(web_dir, fatal=fatal)
    try:
        lock_file = open(_web_ui_stamp_path().parent / ".web_ui_build.lock", "a", encoding="utf-8")
    except OSError:
        return _do_build_web_ui_via_facade(web_dir, fatal=fatal)
    try:
        try:
...
def _do_build_web_ui_via_facade(web_dir: Path, *, fatal: bool = False) -> bool:
    main_mod = sys.modules.get("hermes_cli.main")
    facade_build = getattr(main_mod, "_do_build_web_ui", None) if main_mod is not None else None
    if facade_build is not None and facade_build is not _do_build_web_ui:
        return facade_build(web_dir, fatal=fatal)
