...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    'info' / 'warn' / 'error'. Pure formatting — no I/O — so it is unit-testable
    ...
    if state_db_path.exists():
        ...
        try:
            ...
            for _kind, _text, _detail in _render_state_db_stats(
                ...
                if _kind == "error":
                    check_fail(_text, _detail)
                    issues.append(
                        "state.db search index repair-required — stop the gateway "
                        "and run 'hermes sessions repair-search'"
                    )
                elif _kind == "warn":
