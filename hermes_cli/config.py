...
# path -> (mtime_ns, size, content_hash, raw yaml dict) for read_raw_config() (no defaults merged in).
_RAW_CONFIG_CACHE: Dict[str, Tuple[int, int, str, Dict[str, Any]]] = {}
...
def _read_raw_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        config_path = get_config_path()
        snapshot = _file_content_snapshot(config_path)
        if snapshot is None:
            return {}
        cache_key, raw_bytes = snapshot
        ...
        try:
            data = fast_safe_load(raw_bytes.decode("utf-8")) or {}
        except Exception as e:
            _warn_config_parse_failure(config_path, e)
            return {}
        ...
        _RAW_CONFIG_CACHE[path_key] = (cache_key[0], cache_key[1], cache_key[2], cached_copy)
...
def _security_policy_leaf(raw: Any, name: str, *, source: str) -> bool:
    """Return one strict ``security.<name>`` boolean from an already-parsed mapping."""
    if not isinstance(raw, dict):
        raise ValueError(f"{source} must be a mapping")
    security = raw.get("security")
    if security is None:
        raise KeyError(f"security.{name} is not set in {source}")
    if not isinstance(security, dict):
        raise ValueError(f"security in {source} must be a mapping")
    value = security.get(name)
    if not isinstance(value, bool):
        raise ValueError(f"security.{name} in {source} must be an explicit boolean")
    return value


@dataclass(frozen=True)
class _SecurityPolicyAuthorization:
    name: str
    allowed: bool
    managed_signature: Tuple[int, int, bytes]
    user_signature: Tuple[int, int, bytes]

    def validate(self) -> bool:
        try:
            lease = load_security_policy_bool_strict(
                self.name, default=False, authorization=True
            )
        except Exception:
            return False
        return (
            lease.allowed == self.allowed
            and lease.managed_signature == self.managed_signature
            and lease.user_signature == self.user_signature
        )


def _read_policy_yaml_with_signature(path: Path) -> Tuple[Any, Tuple[int, int, bytes]]:
    st = path.stat()
    raw = path.read_bytes()
    return yaml.safe_load(raw.decode("utf-8")) or {}, (
        st.st_mtime_ns,
        st.st_size,
        __import__("hashlib").blake2b(raw, digest_size=16).digest(),
    )
...
def load_security_policy_bool_strict(
    ...
    *,
    ...
    """Read ``security.<name>`` from the exact policy source, failing closed.

    Managed scope, when explicitly selected, is the only authority for this
    leaf. A returned authorization object revalidates the same file signatures
    immediately before an installer side effect.
    """
    managed_dir_override = os.environ.get("HERMES_MANAGED_DIR", "").strip()
    managed_dir = managed_scope.get_managed_dir()
    if managed_dir_override and managed_dir is None:
        raise RuntimeError("selected managed scope is unavailable")

    managed_signature: Tuple[int, int, bytes] = (0, 0, b"")
    user_signature: Tuple[int, int, bytes] = (0, 0, b"")
    if managed_dir is not None:
        managed_path = managed_dir / "config.yaml"
        if managed_path.is_symlink():
            raise OSError(f"managed config must not be a symlink: {managed_path}")
        raw, managed_signature = _read_policy_yaml_with_signature(managed_path)
        value = _security_policy_leaf(raw, name, source=str(managed_path))
    else:
        user_path = get_config_path()
        if user_path.is_symlink():
            raise OSError(f"config must not be a symlink: {user_path}")
        raw, user_signature = _read_policy_yaml_with_signature(user_path)
        try:
            value = _security_policy_leaf(raw, name, source=str(user_path))
        except KeyError:
            value = default

    if not authorization:
        return value
    return _SecurityPolicyAuthorization(
        name=name,
        allowed=value,
...
def _file_content_snapshot(path: Path) -> Optional[Tuple[Tuple[int, int, str], bytes]]:
    try:
        data = path.read_bytes()
        st = path.stat()
    except (FileNotFoundError, OSError):
        return None
    return (st.st_mtime_ns, st.st_size, hashlib.sha256(data).hexdigest()), data


def _file_content_sig(path: Path) -> Optional[Tuple[int, int, str]]:
    snapshot = _file_content_snapshot(path)
    return snapshot[0] if snapshot is not None else None


def _load_config_cache_snapshot(config_path: Path) -> Tuple[
    Optional[Tuple[int, int, str]],
    Optional[tuple],
    Optional[bytes],
    Optional[bytes],
]:
    """Return ``(user_sig, cache_sig, user_bytes, managed_bytes)`` for ``_LOAD_CONFIG_CACHE``.
    ...
    user_snapshot = _file_content_snapshot(config_path)
    user_sig = user_snapshot[0] if user_snapshot else None
    user_bytes = user_snapshot[1] if user_snapshot else None
    managed_dir = managed_scope.get_managed_dir()
    managed_snapshot = _file_content_snapshot(managed_dir / "config.yaml") if managed_dir else None
    managed_sig = managed_snapshot[0] if managed_snapshot else None
    managed_bytes = managed_snapshot[1] if managed_snapshot else None
    if user_sig is None and managed_sig is None:
        return None, None, None, None
    return user_sig, (user_sig, managed_sig), user_bytes, managed_bytes


def _load_config_cache_sig(config_path: Path) -> Tuple[Optional[Tuple[int, int, str]], Optional[tuple]]:
    user_sig, cache_sig, _user_bytes, _managed_bytes = _load_config_cache_snapshot(config_path)
    return user_sig, cache_sig
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        user_sig, cache_sig, user_bytes, _managed_bytes = _load_config_cache_snapshot(config_path)
        ...
        if user_sig is not None:
            try:
                user_config = fast_safe_load((user_bytes or b"").decode("utf-8")) or {}
