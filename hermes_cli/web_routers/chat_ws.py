...
_assistant_mode_enabled = late("_assistant_mode_enabled")
...
async def _close_unless_sidecar_allowed(ws: WebSocket, *, assistant_gateway: bool = False) -> bool:
    ...
    if not _DASHBOARD_EMBEDDED_CHAT_ENABLED:
        ...
        return False
    if _assistant_mode_enabled() and not assistant_gateway:
        await ws.close(code=4403, reason="websocket disabled in assistant mode")
        return False
...
async def gateway_ws(ws: WebSocket) -> None:
    if not await _close_unless_sidecar_allowed(ws, assistant_gateway=True):
        return
    from tui_gateway.ws import handle_ws
    from hermes_cli import web_server as _server
