...
_restore_custom_endpoint_env = late("_restore_custom_endpoint_env")
_snapshot_custom_endpoint_env = late("_snapshot_custom_endpoint_env")
...
def _write_custom_endpoint(cfg: Dict[str, Any], body: CustomEndpointUpdate) -> Tuple[str, Dict[str, Any]]:
    ...
    if submitted_key:
        if save_env_value(env_var, submitted_key) is False:
            raise RuntimeError("credential persistence refused")
        entry["key_env"] = env_var
        entry.pop("api_key", None)
    elif submitted_key is not None:
        # Blank field means "clear the key", not "leave it alone".
        if remove_env_value(env_var) is False:
            raise RuntimeError("credential removal refused")
        ...
        if save_env_value(env_var, entry["api_key"].strip()) is False:
            raise RuntimeError("credential persistence refused")
        entry["key_env"] = env_var
        entry.pop("api_key", None)
...
def upsert_custom_endpoint(body: CustomEndpointUpdate, profile: Optional[str] = None):
    ...
    with http_failure("POST /api/providers/custom-endpoints failed", 500, detail="Failed to save custom endpoint"):
        with _config_profile_scope(profile):
            env_snapshot = _snapshot_custom_endpoint_env()
            cfg = load_config()
            endpoint_id, _entry = _write_custom_endpoint(cfg, body)
            try:
                save_config(cfg)
            except Exception:
                _restore_custom_endpoint_env(env_snapshot)
                raise
