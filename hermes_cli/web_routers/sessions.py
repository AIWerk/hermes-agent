...
_enforce_cui_session_visible = late("_enforce_cui_session_visible")
...
_project_cui_message_rows_public = late("_project_cui_message_rows_public")
_project_session_list_rows_public = late("_project_session_list_rows_public")
_sanitize_public_message_value = late("_sanitize_public_message_value")
...
def _restricted_actor_denied(actor: dict | None) -> None:
...
def _session_list_row_visible_to_cui_actor(db, row: dict, actor: dict) -> bool:
    ...
    if not projected_id or not lineage_root_id or projected_id == lineage_root_id:
        return True
    projected_row = db.get_session(projected_id)
    return _session_visible_to_cui_actor(projected_row, actor)


def _project_session_detail_public(row: dict) -> dict:
    allowed = {
        "id", "source", "title", "preview", "summary", "topics", "started_at",
        "last_active", "ended_at", "message_count", "profile", "is_default_profile",
    }
    return {key: _sanitize_public_message_value(row.get(key)) for key in allowed if key in row}
...
def get_sessions(
    ...
    profile = _customer_profile(profile, actor)
    ...
    try:
        ...
        try:
            ...
            if actor and not actor.get("_restricted"):
                all_sessions = _list_sessions_rich_all(
                    db,
                ...
                visible = [
                    ...
                    if _session_list_row_visible_to_cui_actor(db, row, actor)
                ]
                total = len(visible)
                sessions = [dict(row) for row in visible[offset: offset + limit]]
            else:
    ...
    with http_failure("GET /api/sessions/search failed", 500, detail="Search failed"):
        ...
        try:
            ...
            def add_lineage_result(raw_sid: str, payload: dict) -> None:
                ...
                if actor and not actor.get("_restricted") and not _session_visible_to_cui_actor(raw_session, actor):
                    return
                ...
                try:
                    ...
                    row = None
                if actor and not actor.get("_restricted") and (not row or not _session_visible_to_cui_actor(row, actor)):
                    return
                if row:
            ...
            id_fetch_limit = safe_limit
            while len(seen) < safe_limit:
                before = len(seen)
                id_rows = db.search_sessions_by_id(
                    q, limit=id_fetch_limit, include_archived=True, source=source_filter,
                ...
                for row in id_rows:
                    ...
                    if len(seen) >= safe_limit:
                        break
                if len(id_rows) < id_fetch_limit or len(seen) == before:
                    break
                id_fetch_limit *= 2
            ...
            while len(seen) < safe_limit:
                ...
                matches = db.search_messages(
                    ...
                    exclude_sources=exclude_list or None, limit=fetch_limit,
                ...
                if len(matches) < fetch_limit or len(seen) == before:
                    break
                fetch_limit *= 2
            ...
            if actor and not actor.get("_restricted"):
                for result in results:
                    if "snippet" in result:
                        result["snippet"] = _sanitize_public_message_value(result.get("snippet"))
                    if "preview" in result:
                        result["preview"] = _sanitize_public_message_value(result.get("preview"))
                results = _project_session_list_rows_public(results)
...
async def bulk_delete_sessions_endpoint(request: Request, body: BulkDeleteSessions):
    ...
    """
    if isinstance(request, BulkDeleteSessions):
    ...
    actor = _cui_actor_context_from_request(request)
    _restricted_actor_denied(actor)
    profile = _customer_profile(body.profile, actor)
    if actor and not actor.get("_restricted"):
        db = _open_session_db_for_profile(profile, read_only=True)
        try:
            for session_id in body.ids:
                _enforce_cui_session_visible(db, session_id, actor)
        finally:
            db.close()
...
async def import_sessions_endpoint(request: Request):
    ...
    _restricted_actor_denied(actor)
    if actor and not actor.get("_restricted"):
        raise HTTPException(status_code=403, detail="Session import is not available to this actor")
    try:
...
async def delete_empty_sessions_endpoint(request: Request, profile: Optional[str] = None):
    ...
    * Active sessions are skipped (``ended_at IS NULL``) so a live agent isn't yanked mid-handshake. *
    ...
    _restricted_actor_denied(actor)
    if actor and not actor.get("_restricted"):
        raise HTTPException(status_code=403, detail="Delete empty sessions is not available to this actor")
    deleted = await asyncio.to_thread(
...
async def get_session_stats(request: Request, profile: Optional[str] = None):
    ...
    _restricted_actor_denied(actor)
    if actor and not actor.get("_restricted"):
        raise HTTPException(status_code=403, detail="Session stats are not available to this actor")
...
async def get_session_detail(request: Request, session_id: str, profile: Optional[str] = None):
    ...
    def _detail(db):
        ...
        if actor and not actor.get("_restricted") and sid:
            _enforce_cui_session_visible(db, sid, actor)
        ...
        if actor and not actor.get("_restricted"):
            return _project_session_detail_public(session)
        return session
...
async def get_session_latest_descendant(request: Request, session_id: str, profile: Optional[str] = None):
    actor = _cui_actor_context_from_request(request)
    _restricted_actor_denied(actor)
    profile = _customer_profile(profile, actor)
    ...
    def _latest(db):
        ...
        if actor and not actor.get("_restricted") and resolved:
            _enforce_cui_session_visible(db, resolved, actor)
        latest, path = _session_latest_descendant(session_id, db)
        if actor and not actor.get("_restricted"):
            for sid in path or ([latest] if latest else []):
                _enforce_cui_session_visible(db, sid, actor)
        return latest, path

    latest, path = await asyncio.to_thread(
        _with_db, profile, _latest, read_only=True)
    if not latest:
        raise HTTPException(status_code=404, detail=_NOT_FOUND)
    return {
...
async def get_session_messages(
    request: Request, session_id: str, profile: Optional[str] = None, limit: Optional[int] = Query(None, ge=0),
    ...
    if actor and not actor.get("_restricted"):
        projected_messages = _project_cui_message_rows_public(projected_messages)
    return {
        "session_id": sid,
...
async def rename_session_endpoint(session_id: str, body: SessionRename, request: Request):
    ...
    return _with_db(profile, _update, read_only=False)
...
async def export_session_endpoint(request: Request, session_id: str, profile: Optional[str] = None):
    ...
    session["is_default_profile"] = session["profile"] == "default"
    export_session = (
        _project_session_detail_public(session)
        if actor and not actor.get("_restricted")
        else session
    )
    ...
    def _stream_export():
        db = _open_session_db_for_profile(profile, read_only=True)
        try:
            yield _compact_json(export_session)[:-1] + ',"messages":['
            ...
            while True:
                ...
                if actor and not actor.get("_restricted"):
                    messages = _project_cui_message_rows_public(_project_for_display(messages))
                for message in messages:
