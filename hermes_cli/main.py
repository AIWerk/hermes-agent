...
# login->profile binding or tenant-profile Step 1.
# Extracted bootstrap call sites preserved in owners:
...
_DASHBOARD_LISTENING_IMPORTED = _dashboard_listening
...
def cmd_dashboard(args):
    ...
    import hermes_cli.main_dashboard as _main_dashboard_mod

    if _dashboard_listening is _DASHBOARD_LISTENING_IMPORTED:
        _route_named_profile_dashboard(args, _headless_backend, _ssh_owner_nonce, _token_file)
    else:
        _original_dashboard_listening = _main_dashboard_mod._dashboard_listening
        _main_dashboard_mod._dashboard_listening = _dashboard_listening
        try:
            _route_named_profile_dashboard(args, _headless_backend, _ssh_owner_nonce, _token_file)
        finally:
            _main_dashboard_mod._dashboard_listening = _original_dashboard_listening
    ...
    start_server(
        ...
        mode="assistant" if getattr(args, "assistant", False) else "admin",
