...
def cmd_dashboard(args):
    ...
    # the `/api/ws` + `/api/pty` sockets. The legacy --tui flag is kept for
    # compatibility; --assistant still selects the restricted AIWerk CUI mode.
...
def main():
    ...
    sessions_subparsers.add_parser(
        "repair-search",
        help="Offline rebuild and verification of a repair-required search index",
        description=(
            "Explicitly rebuild and integrity-check the derived session-search "
            "indexes, then clear repair-required only after verification. "
            "Every gateway and other Hermes process must be stopped first."
        ),
    )
