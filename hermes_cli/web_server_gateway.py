...
def _profile_restart_tenant_id(profile: Optional[str]) -> str:
    """Tenant owner for a dashboard gateway-restart target, when profile config declares one."""
    ...
    candidates = (
        dashboard.get("tenant_id"),
        dashboard.get("org_id"),
        cfg.get("_cui_tenant_id"),
        cfg.get("tenant_id"),
    )
    return next((str(value).strip() for value in candidates if str(value or "").strip()), "")


def _resolve_gateway_restart_target(profile: Optional[str]) -> tuple[Optional[str], str]:
    """Resolve the canonical profile record that policy authorizes and spawn uses."""
    requested = (profile or "").strip()
    if not requested or requested.lower() == "current":
        return None, _profile_restart_tenant_id(None)
    if requested == "default":
        return "default", _profile_restart_tenant_id("default")
    ...
    return requested, _profile_restart_tenant_id(requested)
