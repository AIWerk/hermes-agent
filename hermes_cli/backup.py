...
def _query_ro_sqlite(path: Path, fn):
    ...
    try:
        ...
        conn = connect_tracked(
            f"file:{path}?mode=ro", tracking_path=path,
            connect_fn=sqlite3.connect, uri=True, timeout=1.0)
        return fn(conn), None
    except Exception as exc:
        return None, exc
    finally:
...
def _safe_copy_db(src: Path, dst: Path, *, timeout_seconds: float = 10.0) -> bool:
    ...
    try:
        ...
        conn = connect_tracked(
            f"file:{src}?mode=ro", tracking_path=src,
            connect_fn=sqlite3.connect, uri=True, timeout=0.0)
        backup_conn = connect_tracked(dst, tracking_path=dst, connect_fn=sqlite3.connect)
    ...
    try:
        ...
        for pid_str in os.listdir("/proc"):
            ...
            try:
                ...
                return None
            held = _holds_watched(fds, fd_dir)
            if held is None:
                return None
            if held:
...
def _safe_restore_db(src: Path, dst: Path) -> bool:
    ...
    try:
        ...
        dst_conn = connect_tracked(dst, tracking_path=dst, connect_fn=sqlite3.connect)
        ...
        with closing(connect_tracked(
            f"file:{src}?mode=ro", tracking_path=src,
            connect_fn=sqlite3.connect, uri=True)) as src_conn:
...
def _unlink_move_restore_db(src: Path, dst: Path) -> bool:
    ...
    try:
        ...
        if holders is None:
            logger.error(
                "Refusing unlink+move restore of %s: database opener state is unknown. "
                "This restore requires a Linux /proc scan with no unreadable fd directories.",
                dst,
            )
            return False
        if holders:
