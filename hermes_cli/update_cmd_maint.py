...
def _print_fts_optimize_available_notice() -> None:
    ...
    try:
        ...
        stdout_before_state_import = sys.stdout
        from hermes_state import SessionDB
        if sys.stdout is sys.stderr:
            sys.stdout = stdout_before_state_import
    except Exception:
        return
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    clobbers pages. Unknown holder state refuses closed; this automatic path is Linux-only
    because it depends on a complete ``/proc`` fd scan. Raises OSError if the copy fails.
    """
    from hermes_cli.backup import _foreign_db_holder_pids, verify_sqlite_integrity
    from hermes_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
    # Acquire the cross-process lease before the authoritative opener census and
    # retain it through sidecar clearing, replacement, and integrity verification.
    try:
        with offline_file_access(state_path, what="restore a snapshot over"):
            holders = _foreign_db_holder_pids(state_path)
            if holders is None:
                print(
                    "  ✗ Auto-restore refused: database opener state is unknown. "
                    "Automatic restore requires Linux /proc with readable fd directories; "
                    "restore manually after stopping Hermes."
                )
                return False
            if holders:
                print(
