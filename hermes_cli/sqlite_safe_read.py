...
def _acquire_cross_process_lease(path: Path | str, *, exclusive: bool) -> int | None:
    """Acquire the cooperative per-database flock held by every tracked opener."""
    if fcntl is None:
        return None
    raw = os.fspath(path)
    if raw == ":memory:" or raw.startswith("file:"):
        return None
    key = _key(path)
    if not exclusive and key in getattr(_lease_local, "exclusive_keys", set()):
        # Nested verification opened by the thread that already owns the exclusive
        # offline lease must not self-deadlock on a second flock descriptor.
        return None
    lease_path = Path(f"{key}.hermes-offline.lock")
    ...
    try:
        ...
        if (
            ...
            or metadata.st_uid != os.getuid()  # windows-footgun: ok — fcntl guard returned above
            or metadata.st_nlink != 1
        ):
            raise LiveConnectionError(f"Unsafe SQLite lease file: {lease_path}")
        ...
        try:
            ...
            raise LiveConnectionError(
                f"Refusing offline access to {path}: another Hermes process has the database open"
            ) from exc
        return fd
    except Exception:
        os.close(fd)
...
class _TrackingMixin:
    ...
    _hermes_lease_fd: int | None = None
    ...
    def close(self) -> None:  # type: ignore[misc]
        with _live_lock:
            ...
            lease_fd = getattr(self, "_hermes_lease_fd", None)
            if lease_fd is not None:
                self._hermes_lease_fd = None
                os.close(lease_fd)
...
def connect_tracked(
    ...
    lease_fd = _acquire_cross_process_lease(tracking_path or path, exclusive=False)
    try:
        with _live_lock:
            ...
            try:
                ...
                conn._hermes_lease_fd = lease_fd
                _track_key(resolved)
                return conn
            except Exception:
                try:
...
def offline_file_access(path: Path | str, *, what: str = "read"):
    ...
    lease_fd = _acquire_cross_process_lease(path, exclusive=True)
    exclusive_keys = getattr(_lease_local, "exclusive_keys", None)
    if exclusive_keys is None:
        exclusive_keys = set()
        _lease_local.exclusive_keys = exclusive_keys
    exclusive_keys.add(key)
    try:
        with _live_lock:
            # Recheck after acquiring the cross-process lease: a local opener may
            # have won the interval between the initial check and flock acquisition.
            if key in _live_connections:
                raise LiveConnectionError(
                    f"Refusing to {what} {path}: a connection to it is still open "
                    "in this process, and raw file access would cancel that "
