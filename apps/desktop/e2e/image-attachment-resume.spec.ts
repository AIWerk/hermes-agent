...
// Keep the explicit session title distinct from the caption so the test covers
// both durable session selection and persisted multimodal turn rendering.
...
test.describe('attached image resume', () => {
  ...
  test('renders a persisted attachment as a thumbnail on first open and after a cold reload', async ({}, testInfo) => {
    ...
    // The explicit durable title selects the seeded row; caption persistence is
    // asserted independently in the transcript below.
