...
class SessionMessagesMixin:
            ...
            for _ in range(32):
                try:
                    ...
                    child_row = conn.execute(
                        ...
                        "                  WHERE parent.id = ? AND parent.end_reason = 'side_session') "
                        "  AND json_extract(COALESCE(child.model_config, '{}'), '$._branched_from') IS NULL "
                        "  AND json_extract(COALESCE(child.model_config, '{}'), '$._delegate_from') IS NULL "
                        "  AND COALESCE(json_extract(COALESCE(child.model_config, '{}'), '$._side_from'), '') != ? "
                        ...
                        "ORDER BY child.started_at DESC, child.id DESC LIMIT 1", (current, current, current)).fetchone()
                except Exception:
                    return session_id
    ...
    def _resume_lineage_ids(self, session_id: str) -> List[str]:
        ...
        return [session_id] if self.is_explicit_fork_child(session_id) else self._session_lineage_root_to_tip(session_id)
    ...
    def _is_explicit_fork_child_row(self, session: Dict[str, Any]) -> bool:
        ...
        if not isinstance(cfg, dict):
            return False
        markers = (cfg.get("_branched_from"), cfg.get("_delegate_from"), cfg.get("_side_from"))
