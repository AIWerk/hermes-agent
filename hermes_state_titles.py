...
class SessionTitlesMixin:
    ...
    def get_session_title_metadata(self, session_id: str) -> Dict[str, Any]:
        """Return title text plus the legacy public provenance label."""
        row = self._read_one("SELECT title, title_source FROM sessions WHERE id = ?", (session_id,))
        if not row:
            return {"title": None, "title_source": None}
        source = row["title_source"] if row["title"] is not None else None
        if source == self.TITLE_SOURCE_USER:
            source = "manual"
        return {"title": row["title"], "title_source": source}
