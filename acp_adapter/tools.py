...
def _more(items: list, shown: int, unit: str = "") -> List[str]:
    """Trailer when ``items`` overflow the displayed cap."""
...
def _build_polished_completion_content(
    ...
    if (formatter := _COMPLETION_FORMATTERS.get(tool_name)) is not None:
        ...
        text = _format_generic_structured_result(
            tool_name,
            result,
            fallback_to_text=tool_name in _POLISHED_TOOLS,
        )
...
def _start_todo(args: Args) -> str:
    ...
    lines.extend(
        f"- {i.get('status', 'pending')}: {i.get('content', i.get('id', ''))}"
        for i in items[:8]
...
def _start_delegate(args: Args) -> str:
    ...
    for i, task in enumerate(tasks[:8], 1):
        if isinstance(task, dict):
            lines.append(
                f"{i}. " + _truncate_text(_arg(task, "goal"), limit=160) + _fmt(_arg(task, "role"), " ({})", "")
            )
...
_START_CONTENT_BUILDERS: Dict[str, Optional[Callable[[Args], Any]]] = {
    ...
    "write_file": lambda a: _fmt(
        a.get("path", ""),
        "Preparing write to {}. Approval prompt shows the diff.",
    ...
    "web_extract": None,
    "process": lambda a: f"Process action: {_arg(a, 'action', default='manage')}"
    + _fmt(_arg(a, "session_id"), "\nSession: {}", "")
    ...
    if tool_name in ("patch", "write_file") and edit_diff is not None:
        content = [
            acp.tool_diff_content(
                path=sanitize_tool_display_text(getattr(edit_diff, "path", "")),
                old_text=(
                    sanitize_tool_display_text(edit_diff.old_text)
                    if getattr(edit_diff, "old_text", None) is not None
                    else None
                ),
                new_text=sanitize_tool_display_text(getattr(edit_diff, "new_text", "")),
            )
        ]
