...
from agent.tool_argument_projection import project_tool_args_for_display, sanitize_tool_display_text
...
def _history_message_chunk(role: str, message: dict[str, Any]) -> UserMessageChunk | AgentMessageChunk | None:
    ...
    if not text:
        return None
    text = sanitize_tool_display_text(text)
...
def _history_replay_updates(history: list[dict[str, Any]]):
    ...
    for message in history:
        ...
        if role == "user":
            ...
            if thought:
                yield HermesACPAgent._history_thought_update(thought)
            if (chunk := _history_message_chunk(role, message)) is not None:
                yield chunk
            ...
            yield build_tool_complete(
                tool_call_id,
                tool_name,
                result=result_text,
                function_args=project_tool_args_for_display(tool_name, function_args),
            )
            if tool_name == "todo":
...
class HermesACPAgent(SlashCommandsMixin, acp.Agent):
    ...
    @staticmethod
    def _history_message_update(role: str, text: str) -> UserMessageChunk | AgentMessageChunk | None:
        """Build one sanitized replay chunk for stored user/assistant text."""
        if role not in _HISTORY_CHUNK_TYPES:
            return None
        clean_text = sanitize_tool_display_text(text).strip()
        if not clean_text:
            return None
        cls, session_update = _HISTORY_CHUNK_TYPES[role]
        return cls(
            session_update=session_update,
            content=TextContentBlock(type="text", text=clean_text),
            field_meta=_history_summary_meta({}, clean_text),
        )

    @staticmethod
    def _history_thought_update(text: str) -> Any:
        """Build one sanitized replay chunk for stored reasoning text."""
    ...
    async def _send_session_info_update(
        ...
        update = SessionInfoUpdate(
            ...
            title=sanitize_tool_display_text(title) if isinstance(title, str) and title.strip() else None,
    ...
    async def list_sessions(
        ...
        sessions = [
            SessionInfo(
                ...
                title=(
                    sanitize_tool_display_text(s.get("title"))
                    if isinstance(s.get("title"), str) and s.get("title").strip()
                    else None
                ),
    ...
    async def prompt(self, prompt: list[PromptBlock], session_id: str, **kwargs: Any) -> PromptResponse:
        ...
        if text_only_prompt and isinstance(user_content, str) and user_text.startswith("/"):
            ...
            if response_text is not None:
                if self._conn:
                    update = acp.update_agent_message_text(sanitize_tool_display_text(response_text))
        ...
        with state.runtime_lock:
            if state.is_running and not queued_drain_turn:
                if text_only_prompt and isinstance(user_content, str) and hasattr(state.agent, "redirect") and (
                    getattr(state.agent, "_supports_active_turn_redirect", False) is True
                ):
                    try:
        ...
        if redirected:
            if self._conn:
                await self._conn.session_update(
                    session_id,
                    acp.update_agent_message_text("Redirected the active turn with your correction."),
                )
            return PromptResponse(stop_reason="end_turn")
        if queued_depth is not None:
            if self._conn:
                await self._conn.session_update(
                    session_id,
                    acp.update_agent_message_text(f"Queued for the next turn. ({queued_depth} queued)"),
                )
            return PromptResponse(stop_reason="end_turn")
        ...
        if conn:
            ...
            state.agent.reasoning_callback = _reasoning_cb
            state.agent.stream_delta_callback = stream_delta_cb
        ...
        async def _flush_buffered_customer_streams() -> None:
            if conn and reasoning_fragments:
                await conn.session_update(
                    session_id,
                    acp.update_agent_thought_text(sanitize_tool_display_text("".join(reasoning_fragments))),
                )
                reasoning_fragments.clear()
            if conn and message_fragments:
                await conn.session_update(
                    session_id,
                    acp.update_agent_message_text(sanitize_tool_display_text("".join(message_fragments))),
                )
        ...
        async def _drain_queued_prompts(*, owns_turn: bool) -> None:
            ...
            try:
                if conn:
                    await conn.session_update(
                        session_id,
                        acp.update_user_message_text(sanitize_tool_display_text(next_prompt)),
                    )
                await self.prompt(
        ...
        async def _finish_completed_queued_turn() -> None:
            drain_task = loop.create_task(_drain_queued_prompts(owns_turn=True))
        ...
        # The provider turn is complete here.
        await _flush_buffered_customer_streams()
        ...
        if final_response and conn and not suppress and (not streamed_message or result.get("response_transformed")):
            update = acp.update_agent_message_text(sanitize_tool_display_text(final_response))
            try:
