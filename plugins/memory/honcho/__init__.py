...
class HonchoMemoryProvider(DialecticMixin, MemoryProvider):
    ...
    def __init__(self, query_rewriter: Optional[Callable[[str], str]] = None):
        ...
        self._runtime_user_peer_name: Optional[str] = None
        self._runtime_user_peer_name_alt: Optional[str] = None
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        if kwargs.get("user_id") is not None:
            self._runtime_user_peer_name = kwargs.get("user_id")
        if kwargs.get("user_id_alt") is not None:
            self._runtime_user_peer_name_alt = kwargs.get("user_id_alt")
        self._manager = HonchoSessionManager(
            honcho=get_honcho_client(cfg), config=cfg, context_tokens=cfg.context_tokens,
            runtime_user_peer_name=kwargs.get("user_id") or self._runtime_user_peer_name or None,
            runtime_user_peer_name_alt=kwargs.get("user_id_alt") or self._runtime_user_peer_name_alt or None,
        )
        self._session_key = self._resolve_session_key(cfg, session_id, **kwargs)
        ...
        if (
            ...
            and not self._recall_sync
            and self._injection_flag("includeDialectic", True)
        ):
    ...
    def on_session_end(self, messages: List[Dict[str, Any]]) -> None:
        ...
        if not self._session_initialized and self._init_thread and self._init_thread.is_alive():
            self._init_thread.join(timeout=float(getattr(self._config, "timeout", 10.0) or 10.0))
        if not self._manager:
            return
        if self._sync_thread and self._sync_thread.is_alive():
