...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def _reply_context(self, message: Message) -> tuple[Optional[str], Optional[str]]:
        """Return ``(reply_to_id, reply_to_text)`` for the replied-to message."""
    ...
    def _build_message_event(
        ...
        telegram_chat_type = self._chat_type_str(chat)
        chat_type = (
            "group"
            if telegram_chat_type in {"group", "supergroup"}
            else ("channel" if telegram_chat_type == "channel" else "dm")
        )
        ...
        source = self.build_source(
            chat_id=str(chat.id),
            chat_name=chat.title or (chat.full_name if has_full_name else None),
            chat_type=chat_type,
