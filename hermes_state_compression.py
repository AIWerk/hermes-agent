...
def _compression_child_model_config(model_config: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    if not model_config:
        return None
    cleaned = dict(model_config)
    cleaned.pop("_side_from", None)
...
class SessionCompressionMixin:
    ...
    def _publish_child_session_row(self, conn, parent, *, parent_session_id, child_session_id, source,
        ...
        conn.execute(
            ...
            (
                child_session_id, source, model, json.dumps(_compression_child_model_config(model_config)) if model_config else None,
    ...
    def get_compression_lineage(self, session_id: str) -> List[str]:
        ...
        if (
            not session
            or self._is_explicit_fork_child_row(session)
            or self._read_one(
                "SELECT 1 FROM session_stack WHERE side_session_id = ? LIMIT 1",
                (session_id,),
            )
