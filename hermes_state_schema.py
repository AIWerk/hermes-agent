...
class SessionSchemaMixin:
    ...
    def _recover_stale_fts_locked(self, cursor: sqlite3.Cursor, *, legacy: bool) -> bool:
        ...
        try:
            ...
            # FTS table with canonical rank=1 external-content integrity commands.
            self._ensure_fts_cjk_schema(cursor)
            verified, detail = self._verify_fts_repair(self._conn)
            if not verified:
                raise sqlite3.DatabaseError(f"FTS verification failed after automatic rebuild: {detail}")
            cursor.execute(
                "DELETE FROM state_meta WHERE key IN (?, ?)",
                (FTS_STALE_KEY, FTS_REBUILD_DEFERRAL_KEY),
            )
            self._conn.commit()
        except sqlite3.DatabaseError as exc:
            ...
            with contextlib.suppress(sqlite3.Error):
                ...
                self._conn.commit()
            logger.error(
                "Automatic rebuild of stale FTS indexes failed or could not be verified (%s); "
