...
class SessionPortabilityMixin:
    ...
    @classmethod
    def _compact_session_cols(cls, visibility: str = "public") -> str:
        """Return the explicit compact projection for one visibility boundary.

        ``public`` contains only customer-display metadata. ``internal`` adds
        fields needed by trusted admin/routing consumers but still excludes
        secret blobs. New schema columns are not included automatically: the
        schema-coverage test requires a deliberate classification first.
        """
        if visibility not in {"public", "internal"}:
            raise ValueError(f"unsupported compact session visibility: {visibility}")
        cached = cls._session_compact_cols_sql.get(visibility)
        if cached is None:
            admitted = {"public"} if visibility == "public" else {"public", "internal"}
            declared = cls._parse_schema_columns(SCHEMA_SQL)["sessions"]
            cached = ", ".join(
                f"s.{name}"
                for name in declared
                if cls._SESSION_COLUMN_VISIBILITY.get(name) in admitted
            )
            cls._session_compact_cols_sql[visibility] = cached
        return cached
    ...
    def _get_session_rich_rows_batch(
        ...
        _sel = (
            self._compact_session_cols(compact_visibility)
            if compact_rows
            else "s.*"
        )
