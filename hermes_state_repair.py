...
def _existing_malformed_backups(db_path: Path) -> "List[Path]":
    ...
    try:
        found = [
            ...
            and ".hermes-offline.lock" not in p.name
        ]
    except OSError:
        return []
...
def _unlink_db_triple(path: Path) -> Optional[str]:
    """Remove a disposable DB, sidecars, and its cooperative lease; return any failure."""
    from hermes_state import _IS_WINDOWS
    failures: List[str] = []
    lease = Path(f"{path}.hermes-offline.lock")
    for victim in (path, *_sidecars(path), lease):
