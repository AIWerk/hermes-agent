...
from hermes_state_common import (
    FTS_REPAIR_FIRST_ERROR_AT_KEY, FTS_REPAIR_FAILURE_COUNT_KEY,
    FTS_REPAIR_LAST_ERROR_AT_KEY, FTS_REPAIR_LAST_ERROR_KEY, FTS_REPAIR_META_KEYS,
...
from hermes_state_dbfile import _connect_tracked_db, count_db_holders
from hermes_state_wal import apply_database_pragmas, apply_wal_with_fallback
from hermes_state_fts import FTS_CJK_TABLE_SQL, FTS_CJK_TRIGGER_SQL, load_fts5_cjk_extension
...
class SessionSearchMixin:
    ...
    def _ensure_cjk_schema_committed(self) -> None:
        with self._lock:
            self._raise_if_db_corrupt()  # type: ignore[attr-defined]
            self._raise_if_db_replaced()  # type: ignore[attr-defined]
    ...
    def _repair_optimize_bookkeeping(self) -> None:
        ...
        def _do(conn):
            if _meta_row(conn, "fts_rebuild_high_water") is not None:
                progress = _meta_row(conn, "fts_rebuild_progress")
                if (
                    progress is not None
                    and int(progress[0]) == 0
                    and not self._fts_index_known_empty(conn)
                ):
                    # A zero-progress claim requires a known-empty replay surface.
                    # Reset and include any messages appended since the old claim.
                    self._reset_fts_index_to_empty(conn)
    ...
    def _optimize_settle(self, conn) -> Optional[str]:
        ...
        if self._fts_external_index_empty_with_messages(conn):
            return "backfill_incomplete"
        verified, detail = self._verify_fts_repair(conn)
        if not verified:
            logger.error("FTS storage optimization integrity verification failed: %s", detail)
    ...
    def _finalize_search_matches(
        ...
        repair_required = bool(getattr(self, "_fts_stale", False))
        degraded = repair_required or not bool(getattr(self, "_fts_enabled", False))
        for match in matches:
            match["search_provenance"] = {
                "degraded": degraded,
                "repair_required": repair_required,
                "engine": "canonical_like" if degraded else "fts",
            }
    ...
    def fts_health_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "repair_command": "hermes sessions repair-search" if repair_required else None,
        }
    ...
    def _repair_stale_cjk_fts_offline(self, conn: sqlite3.Connection) -> bool:
        ...
        if not stale:
            return False
        if not self._fts_cjk_loaded:
            raise sqlite3.DatabaseError("CJK repair is required but cjk_unicode61 is unavailable")
        cursor = conn.cursor()
        ...
        cursor.execute("INSERT INTO messages_fts_cjk(messages_fts_cjk) VALUES('rebuild')")
        self._execute_script_in_current_transaction(cursor, FTS_CJK_TRIGGER_SQL)
        cursor.execute(
            "DELETE FROM state_meta WHERE key IN (?, ?, ?)",
            (FTS_CJK_STALE_KEY, "fts_cjk_rebuild_high_water", "fts_cjk_rebuild_progress"),
        )
        self._fts_cjk_available = True
        return True
    ...
    def _fts_schema_uses_external_content(schema_sql: str) -> bool:
        ...
        while i < len(schema_sql):
            ...
            if char.isalpha() or char == "_":
                ...
                while end < len(schema_sql) and (schema_sql[end].isalnum() or schema_sql[end] == "_"):
                    end += 1
        ...
        for index in range(len(tokens) - 2):
            if [token for token, _ in tokens[index:index + 3]] == ["using", "fts5", "("]:
        ...
        for argument in arguments:
            option = re.match(r"^\s*(?:content|[\"`\[]content[\"`\]])\s*=\s*(.*?)\s*$", argument, re.IGNORECASE | re.DOTALL)
            if not option:
                continue
            value = option.group(1).strip()
            if not value:
                raise sqlite3.DatabaseError("empty unquoted FTS5 content option")
            if len(value) >= 2 and ((value[0] == value[-1] and value[0] in "'\"`") or (value[0] == "[" and value[-1] == "]")):
    ...
    def _verify_fts_repair(self, conn: sqlite3.Connection) -> Tuple[bool, str]:
        try:
            ...
            for table in self._FTS_TABLES:
                ...
                if not schema_sql:
                    raise sqlite3.DatabaseError(f"cannot determine FTS content shape for {table}")
                if self._fts_schema_uses_external_content(str(schema_sql)):
    ...
    def _restore_writer_after_offline_repair(self) -> Optional[str]:
        ...
        new_conn = None
        try:
            from hermes_state import _connect_tracked_db as _facade_connect_tracked_db
            new_conn = _facade_connect_tracked_db(str(self.db_path), check_same_thread=False, timeout=1.0, isolation_level=None)
            new_conn.row_factory = sqlite3.Row
            self._conn = new_conn
            self._wal_active = apply_wal_with_fallback(new_conn, db_label="state.db") == "wal"
            ...
            if not isinstance(exc, Exception):
                raise
            logger.error("Verified offline FTS repair completed, but writer connection restoration failed: %s", exc)
    ...
    def repair_fts_offline(self) -> Dict[str, Any]:
        ...
        if holders != 1:
            return {"repaired": False, "verified": False, "reason": f"offline-holder-check-failed:{holders}"}
        ...
        with self._lock:
            ...
            try:
                self._raise_if_db_corrupt()  # type: ignore[attr-defined]
                self._raise_if_db_replaced()  # type: ignore[attr-defined]
                mode = self._conn.execute("PRAGMA locking_mode=EXCLUSIVE").fetchone()
                if not mode or str(mode[0]).lower() != "exclusive":
                    raise sqlite3.OperationalError(f"could not enter EXCLUSIVE locking mode: {mode!r}")
                legacy = self._db_has_legacy_inline_fts(self._conn.cursor())
                stage = "rebuild-indexes"
                rebuilt = self._recover_stale_fts(self._conn.cursor(), legacy=legacy)
                if not rebuilt:
                    return {"repaired": False, "verified": False, "reason": "rebuild-failed"}
                ...
                self._trigram_available = False
                self._fts_cjk_available = False
                return {"repaired": False, "verified": False, "reason": f"{stage}: {exc}"}
            finally:
                try:
        ...
        report = {"repaired": True, "verified": True, "indexes_rebuilt": rebuilt, "cjk_rebuilt": cjk_rebuilt, "connection_restored": restore_error is None}
    ...
    def _search_messages_impl(
        ...
        if is_cjk:
            ...
            try:
                ...
                # Corruption parent class: try one rebuild, then detach the derived indexes and answer from
                # canonical rows.
    ...
    def rebuild_fts(self) -> int:
        ...
        with fts_rebuild_admission(self.db_path) as admitted:
            if not admitted:
                ...
                return 0
            with self._lock:
                # Admission can block while another process replaces/quarantines
                # the live file. Revalidate at the final DML boundary.
                self._raise_if_db_corrupt()  # type: ignore[attr-defined]
                self._raise_if_db_replaced()  # type: ignore[attr-defined]
