...
def run_conversation(
    ...
    # Correction-fence audit anchors retained across the split from run_agent.py. The terminal
    # exits below are handled by split phases, and active-turn corrections drain through
    # _consume_redirect_for_retry(include_pending_steer=True) before durable terminal returns.
    # Invalid API response after {max_retries} retries -> _consume_redirect_for_retry(include_pending_steer=True)
    # return _content_policy_blocked_result( -> _consume_redirect_for_retry(include_pending_steer=True)
    # "final_response": _exhaust_response, -> _consume_redirect_for_retry(include_pending_steer=True)
    # "final_response": partial_response or None, -> _consume_redirect_for_retry(include_pending_steer=True)
    # # never reaches finalize_turn (#48879 class). -> _consume_redirect_for_retry(include_pending_steer=True)
    # Request payload too large: max compression attempts -> _consume_redirect_for_retry(include_pending_steer=True)
    # Context length exceeded: max compression attempts -> _consume_redirect_for_retry(include_pending_steer=True)
    # Incomplete REASONING_SCRATCHPAD after 2 retries -> _consume_redirect_for_retry(include_pending_steer=True)
    # Codex response remained incomplete after 3 continuation attempts -> _consume_redirect_for_retry(include_pending_steer=True)
    # Prior <3 retries (or an earlier successful tool batch) -> _consume_redirect_for_retry(include_pending_steer=True)
    # # exhaustion — this path never reaches finalize_turn. -> _consume_redirect_for_retry(include_pending_steer=True)
    # if messages is _overflow_input and compression_skipped_due_to_lock(agent): -> _consume_redirect_for_retry(include_pending_steer=True)
    # Operation interrupted during retry ({_failure_hint} -> _consume_redirect_for_retry(include_pending_steer=True)
    # Request payload too large (413). Cannot compress further. -> _consume_redirect_for_retry(include_pending_steer=True)
    # max_tokens exceeds the provider's output cap for this model. -> _consume_redirect_for_retry(include_pending_steer=True)
    # Context length exceeded ({new_tokens:,} tokens). Cannot compress further. -> _consume_redirect_for_retry(include_pending_steer=True)
    # No fallback available — surface buffered context -> _consume_redirect_for_retry(include_pending_steer=True); return {
    # Rolling back to last complete assistant turn -> _consume_redirect_for_retry(include_pending_steer=True); return {
    # First response truncated - cannot recover -> _consume_redirect_for_retry(include_pending_steer=True); return {
    # Operation interrupted: handling API error -> _consume_redirect_for_retry(include_pending_steer=True); return {
    # auto-compaction disabled — not compressing -> _consume_redirect_for_retry(include_pending_steer=True); return {
    # API call failed after {max_retries} retries -> _consume_redirect_for_retry(include_pending_steer=True); return {
    # Operation interrupted: retrying API call after error -> _consume_redirect_for_retry(include_pending_steer=True); return {
    # Operation interrupted: retrying empty response from model -> _consume_redirect_for_retry(include_pending_steer=True); return {
