...
def run_conversation(
    ...
    def _consume_redirect_for_retry(*, include_pending_steer: bool = False) -> bool:
        nonlocal original_user_message, final_response
        if include_pending_steer:
            text = agent._take_text_commit_correction_and_close_admission()
        else:
            text = agent._take_pending_redirect_and_close_response_window()
        if not text:
            return False
        _apply_active_turn_redirect(agent, messages, text)
        if isinstance(original_user_message, str):
            original_user_message = (
                f"{original_user_message}\n\n"
                f"User correction during the turn: {text}"
            )
        agent._persist_session(messages, conversation_history)
        final_response = None
        if include_pending_steer:
            # The closed text response was discarded for a same-turn retry.
            agent._open_turn_correction_admission()
        return True

    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        _consume_redirect_for_retry()
        ...
        while retry_count < max_retries:
            ...
            if agent.provider == "nous":
                try:
                    ...
                    if _nous_remaining is not None and _nous_remaining > 0:
                        ...
                        agent._flush_status_buffer()
                        if _consume_redirect_for_retry(include_pending_steer=True):
                            continue
                        agent._persist_session(messages, conversation_history)
                        return {
                            "final_response": (
            ...
            try:
                ...
                try:
                    ...
                    _response_returned = True
                finally:
                    if _redirect_lock is not None:
                        with _redirect_lock:
                            if _model_request_active is not None:
                                _model_request_active.clear()
                            _response_commit_pending = getattr(
                                agent, "_response_commit_pending", None
                            )
                            if _response_returned and _response_commit_pending is not None:
                                _response_commit_pending.set()
                            _redirect_crossed_response = bool(
                                agent._pending_redirect
                            )
                            if _redirect_crossed_response and _response_commit_pending is not None:
                                _response_commit_pending.clear()
                    else:
                        if _model_request_active is not None:
                            _model_request_active.clear()
                        _response_commit_pending = getattr(
        ...
        try:
            ...
            if agent.api_mode == "codex_responses" and finish_reason == "incomplete":
                ...
                # This terminal incomplete-response path is still a response
                # boundary. Atomically close/drain correction admission before
                # persistence/relay; an admitted correction retries this same
                # logical turn instead of being discarded by outer cleanup.
                if _consume_redirect_for_retry(include_pending_steer=True):
                    continue
                agent._persist_session(messages, conversation_history)
                return {
            ...
            if assistant_message.tool_calls:
                # Atomic same-turn boundary: a correction accepted after the
                # provider returned wins before any returned tool can execute.
                if _consume_redirect_for_retry():
                    continue
                if not agent.quiet_mode:
                ...
                # Atomic text-commit boundary. Until this point a CUI redirect
                # still retries this logical turn; after it closes, a later
                # message is legitimately a new queued turn. A steer has no
                # tool-result boundary on this path, so it becomes the same
                # user-side correction representation here.
                if _consume_redirect_for_retry(include_pending_steer=True):
                    continue
