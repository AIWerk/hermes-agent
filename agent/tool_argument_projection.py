...
Execution and persistence retain the complete argument mapping. Customer-facing
CUI/ACP payloads must pass through the explicit per-tool allowlist below.
Unknown tools export no argument values.
"""
...
# Keys are display fields, not execution schemas. Adding a tool or field requires
# an explicit visibility decision here; an unknown tool is deliberately empty.
_TOOL_DISPLAY_ARG_ALLOWLIST: dict[str, frozenset[str]] = {
    ...
    "todo": frozenset({"merge"}),
    "memory": frozenset({"action", "target"}),
    "session_search": frozenset({"query", "limit", "sort", "session_id", "around_message_id", "window", "role_filter", "profile"}),
    "delegate_task": frozenset({"goal", "role"}),
    "skill_view": frozenset({"name", "file_path"}),
    "skills_list": frozenset({"category"}),
    "skill_manage": frozenset({"action", "name", "file_path", "category", "replace_all", "absorbed_into"}),
    "cronjob": frozenset({"action", "job_id", "schedule", "name", "repeat", "deliver", "no_agent", "attach_to_session"}),
    "clarify": frozenset({"question", "multi_select"}),
    "tool_search": frozenset({"query", "limit"}),
    "tool_describe": frozenset({"name"}),
    # Deferred tool arguments are nested and tool-specific; export only its name.
    "tool_call": frozenset({"name"}),
    "computer_use": frozenset({"action", "mode", "app", "element", "coordinate", "direction", "amount", "keys", "delivery_mode", "browser_pointer_action", "browser_dialog_action"}),
}

_SENSITIVE_KEY_FRAGMENTS = (
    "api_key", "apikey", "authorization", "cookie", "credential", "password",
    "prompt_text", "secret", "token", "private_key", "session_cookie",
)
_MAX_DEPTH = 6
_MAX_ITEMS = 100
...
def _redact_text(value: str) -> str:
    try:
        ...
        # URL userinfo is never useful customer-facing context. Strip the whole
        # identity segment rather than retaining a potentially private username.
        ...
        # A display sanitizer failure must not turn into raw export.
        return "[REDACTED]"


def sanitize_tool_display_text(value: Any) -> str:
    """Redact one customer-facing text payload outside an argument mapping."""
...
def project_tool_args_for_display(tool_name: str, args: Any) -> dict[str, Any]:
    """Return the explicit safe display projection for one tool call.

    The caller must retain ``args`` separately for execution/edit-diff logic.
    Unknown tools and malformed argument objects fail closed to an empty mapping.
    """
