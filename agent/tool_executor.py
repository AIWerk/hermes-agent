    ...
    *,
    ...
    if block_message is None:
        try:
            ...
            if memory_write_blocked_for_cui_admin(ref.name, ref.args):
                _advance_start_order()
                state.blocked = True
                result = cui_admin_memory_block_result(ref.name)
                ref.emit_post(
                    agent,
                    result,
                    status="blocked",
...
def _emit_tool_complete_and_risk(agent, ref: _ToolCallRef, result, risk_metadata, blocked: bool) -> None:
    ...
    if not blocked and agent.tool_complete_callback:
        try:
            display_args = _redact_tool_args_for_display(ref.name, ref.args)
