...
def init_agent(
    ...
    # Open only after a provider response returns and until the loop atomically
    # chooses to execute its tools or commit its final assistant row. Redirects
    # in this window still belong to the same logical turn, but no longer abort
    # a request that has already completed.
    agent._response_commit_pending = threading.Event()
    # Guarded by _pending_redirect_lock so final text commit can close steer
    # admission and drain a pending correction atomically.
