...
def _resolve_api_mode(agent, api_mode, provider_name, base_url):
    """Set ``agent.api_mode`` (and provider rewrites) using the upstream routing ladder."""
...
def _set_defaults(agent, table: Dict[str, Any]) -> None:
    """Assign each ``name -> value`` on ``agent``; callables are factories."""
...
_CONTROL_STATE: Dict[str, Any] = {
    ...
    "_response_commit_pending": threading.Event,
    "_steer_admission_open": False,
