...
def redact_tool_args_for_display(tool_name: str, args: dict | None) -> dict | None:
    ...
    if not isinstance(args, dict):
        return args
    if tool_name not in _PRIMARY_ARGS and tool_name not in _PREVIEW_BUILDERS:
        return {}
    if tool_name in {"write_file", "patch"}:
        return {"path": args["path"]} if "path" in args else {}
...
def _preview_process_manage(args: dict, _max_len: int) -> str | None:
    ...
    data_preview = f'"{_oneline(data[:20])}"' if isinstance(data, str) and data else ""
    parts = [str(action) if action else "", str(sid)[:16] if sid else "",
             data_preview, f"{timeout_val}s" if timeout_val and action == "wait" else ""]
...
def build_tool_preview(tool_name: str, args: dict, max_len: int | None = None) -> str | None:
    ...
    *max_len* ``None`` defers to the global ``_tool_preview_max_len``; ``0`` means unlimited.
    ...
    if tool_name not in _PRIMARY_ARGS:
        return None
