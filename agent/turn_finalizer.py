...
def _guarded_cleanup(label: str, fn: Callable[[], Any], errors: List[str], logger) -> None:
    """Post-loop cleanup must never lose the response; errors are surfaced later."""
    try:
        fn()
...
def _resolve_budget_fallback(
    ...
    """Iteration-budget exhaustion. Returns ``(final_response, exit_reason, preserved)``."""
    budget_exhausted = (
...
def finalize_turn(
    ...
    def _persist_step():
        ...
        _record_notes = getattr(agent, "_record_incremental_session_notes", None)
        if callable(_record_notes):
            try:
                _record_notes(messages, conversation_history, final_response)
            except Exception as exc:
                logger.debug(
