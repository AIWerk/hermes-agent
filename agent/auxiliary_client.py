...
class _ChatStreamAccumulator:
    ...
    def remaining_seconds(self) -> Optional[float]:
        ...
        if self._total_ceiling is not None:
            candidates.append(self._total_ceiling - (now - self._started))
        if self._host_deadline is not None:
            candidates.append(self._host_deadline - now)
        return min(candidates) if candidates else None

    def raise_timeout(self) -> NoReturn:
        self._check_deadlines()
        ceiling = self._total_ceiling if self._total_ceiling is not None else 0
        raise TimeoutError(
            f"Auxiliary streamed call timed out after {ceiling:.0f}s "
            "total ceiling (stream still open but over budget)"
        )
...
def _call_llm_impl(
    ...
    _create = _sync_progress_create_for(
        client, task, req.resolved_provider, req.base_info or req.resolved_base_url,
    )
    try:
        ...
        def _perform(step: _LadderStep) -> Any:
            ...
            return _call_fallback_candidate_sync(fb_client, fb_model, fb_label, **kw)
...
async def _async_call_llm_impl(
    ...
    try:
        ...
        result = await _drive_ladder_async(
            _start_recovery_ladder(
                first_err, req, retry_kwargs, task=task, async_mode=True, route_info=route_info
            ),
            _perform,
        )
        if result is _RERAISE_ORIGINAL:
