...
# AIWerk retained prompt-policy composition is enforced in the stable prompt tier.
...
def _platform_hint(agent: Any) -> str:
    """Built-in/plugin platform hint + Telegram rich-messages opt-in + config override."""
...
def _guidance_parts(agent: Any) -> List[str]:
    ...
    if agent.valid_tool_names:
        parts += [
            text for flag, text in (
                ("_information_retrieval_guidance", INFORMATION_RETRIEVAL_GUIDANCE),
    ...
    if "verify_operator_identity" in (agent.valid_tool_names or set()):
        parts.append(OPERATOR_VERIFICATION_GUIDANCE)
    ...
    if actor_prompt := cui_actor_system_prompt():
        parts.append(actor_prompt)
