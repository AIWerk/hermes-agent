...
def _restore_overpruned_tools_submodule(name: str) -> None:
    """Keep split tools modules singletons when callers prune by prefix."""
    if name in sys.modules:
        return
    parent = sys.modules.get("tools")
    module = getattr(parent, name.rsplit(".", 1)[-1], None) if parent is not None else None
    if getattr(module, "__name__", None) == name:
        sys.modules[name] = module


class _OverprunedToolsSubmoduleFinder(importlib.abc.MetaPathFinder, importlib.abc.Loader):
    _NAMES = frozenset({"tools.browser_tool_install"})

    def find_spec(self, fullname, path=None, target=None):
        if fullname not in self._NAMES or fullname in sys.modules:
            return None
        parent = sys.modules.get("tools")
        module = getattr(parent, fullname.rsplit(".", 1)[-1], None) if parent is not None else None
    ...
    def create_module(self, spec):
        parent = sys.modules.get("tools")
        return getattr(parent, spec.name.rsplit(".", 1)[-1], None)

    def exec_module(self, module):
        sys.modules[module.__name__] = module


def _install_overpruned_tools_submodule_finder() -> None:
    if not any(isinstance(finder, _OverprunedToolsSubmoduleFinder) for finder in sys.meta_path):
        sys.meta_path.insert(0, _OverprunedToolsSubmoduleFinder())


_install_overpruned_tools_submodule_finder()
_restore_overpruned_tools_submodule("tools.browser_tool_install")
