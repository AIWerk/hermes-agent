...
def _persist_projected_messages(agent, turn, messages: List[Dict[str, Any]]) -> None:
    """Splice codex app-server projected messages into ``messages`` and flush new rows."""
    ...
    if (
        submitted_user_text is not None
...
def _finish_codex_turn(
    ...
    *,
    ...
    """Post-turn bookkeeping that the app-server path bypasses in the normal loop."""
    ...
    should_review_skills = (
        0 < agent._skill_nudge_interval <= agent._iters_since_skill
    ...
    if not turn.interrupted and turn.error is None:
        _call_guarded(
            getattr(agent, "_sync_external_memory_for_turn", None),
            "external memory sync raised",
            kwargs=dict(
    ...
    if turn.final_text and not turn.interrupted and (should_review_memory or should_review_skills):
        _call_guarded(
            ...
            "background review spawn raised",
            kwargs=dict(
...
def run_codex_app_server_turn(
    ...
    *,
    ...
    """Hand one turn to the codex app-server subprocess and project its events into Hermes."""
    ...
    try:
        ...
        _user_interrupted, _interrupt_message = _consume_user_interrupt(agent)
        retried = _retry_admitted_correction()
        if retried is not None:
            return retried
        return _turn_result(
            (_user_interrupted, _interrupt_message),
            messages,
            api_calls=0,
            completed=False,
            error=str(exc),
