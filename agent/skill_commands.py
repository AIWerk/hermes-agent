...
try:
    ...
    from agent.skill_utils import get_disabled_skill_names as _current_disabled_skill_names

    if not hasattr(_skills_tool_module, "_get_disabled_skill_names"):
        _skills_tool_module._get_disabled_skill_names = _current_disabled_skill_names
except Exception:
    pass


def _install_skill_view_memory_provider_prune_adapter() -> None:
    """Keep memory-provider plugin skills bound to the active provider."""
    try:
        import tools.skills_tool as skills_tool
    except Exception:
        return
    if getattr(skills_tool, "_agent_memory_provider_prune_adapter", False):
        return
    original_resolve = getattr(skills_tool, "_resolve_plugin_skill", None)
    if not callable(original_resolve):
        return

    def _resolve_plugin_skill(name, file_path, task_id, preprocess):
        served, local_category_name = original_resolve(name, file_path, task_id, preprocess)
        if served is None or ":" not in str(name):
            return served, local_category_name
        namespace = str(name).partition(":")[0]
        try:
            from hermes_cli.plugins import get_plugin_manager
            from plugins import memory as memory_plugins

            active = memory_plugins._get_active_memory_provider()
            tracker = getattr(memory_plugins, "_REGISTERED_MEMORY_PROVIDER_SKILLS", {})
            if (
                namespace != active
                and str(name) in tracker
                and get_plugin_manager().find_plugin_skill(str(name)) == tracker[str(name)]
            ):
                memory_plugins._prune_inactive_memory_provider_skills(active)
                return None, local_category_name
        except Exception:
            pass
        return served, local_category_name

    skills_tool._resolve_plugin_skill = _resolve_plugin_skill
    skills_tool._agent_memory_provider_prune_adapter = True


_install_skill_view_memory_provider_prune_adapter()
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    ...
    try:
        ...
        _disabled_fn = getattr(_skills_tool, "_get_disabled_skill_names", get_disabled_skill_names)
        ...
        # See #67277.
        skills_dir = _skills_tool._skills_dir()
...
def _install_tui_gateway_cui_context_facade_adapter() -> None:
    """Restore CUI context helpers on the TUI gateway facade for split callers."""
    try:
        import sys
    ...
    if server is None:
        return
    if not hasattr(server, "bind_cui_actor_context"):
        server.bind_cui_actor_context = bind_cui_actor_context
    if not hasattr(server, "reset_cui_actor_context"):
        server.reset_cui_actor_context = reset_cui_actor_context


_install_tui_gateway_cui_context_facade_adapter()
