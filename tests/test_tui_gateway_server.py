...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    ...
    # Sorted: ["kanban", "memory", "project", "security"]. `kanban` is
    # auto-recovered by _get_platform_tools (a non-configurable platform toolset
    # in hermes-cli's universe); `project` is GUI-only, folded in by
    # _load_enabled_toolsets; `security` carries verify_operator_identity.
    assert server._load_enabled_toolsets() == ["kanban", "memory", "project", "security"]
