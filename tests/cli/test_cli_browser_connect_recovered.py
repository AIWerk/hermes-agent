...
class TestChromeDebugLaunch:
    ...
    def test_connect_context_note_keeps_normal_approval_flow(self, monkeypatch):
        ...
        # Patch the concrete default-loopback discovery path so no real network
        # probe or browser launch happens on the test runner.
        with patch("hermes_cli.cli_commands_mixin.discover_local_cdp_url", return_value="http://127.0.0.1:9222"), \
    ...
    def test_connect_falls_back_when_no_launcher_config(self, monkeypatch):
        ...
        with patch("hermes_cli.cli_commands_mixin.load_local_browser_launcher_config", return_value=load_local_browser_launcher_config({"browser": {}})), \
             patch("hermes_cli.cli_commands_mixin.is_browser_debug_ready", side_effect=fake_ready), \
             patch("hermes_cli.cli_commands_mixin.discover_local_cdp_url", side_effect=[None, "http://127.0.0.1:9222"]), \
             patch("hermes_cli.cli_commands_mixin.wait_for_browser_debug_ready", return_value=True), \
             patch("hermes_cli.cli_commands_mixin.launch_chrome_debug", return_value=ChromeDebugLaunch(launched=True)) as launch, \
             patch("tools.browser_tool.cleanup_all_browsers"), \
             patch("tools.browser_tool._ensure_cdp_supervisor"), \
             redirect_stdout(StringIO()):
            cli._handle_browser_command("/browser connect")
    ...
    def test_connect_falls_back_to_builtin_launch_when_configured_launcher_unreachable(self, monkeypatch):
        ...
        with patch("hermes_cli.cli_commands_mixin.load_local_browser_launcher_config", return_value=cfg), \
             ...
             patch("hermes_cli.cli_commands_mixin.discover_local_cdp_url", side_effect=[None, "http://127.0.0.1:9222"]), \
             patch("hermes_cli.cli_commands_mixin.call_local_browser_launcher", return_value=(False, "connection refused")) as launcher, \
             patch("hermes_cli.cli_commands_mixin.wait_for_browser_debug_ready", return_value=True), \
             patch("hermes_cli.cli_commands_mixin.launch_chrome_debug", return_value=ChromeDebugLaunch(launched=True)) as launch, \
             patch("tools.browser_tool.cleanup_all_browsers"), \
             patch("tools.browser_tool._ensure_cdp_supervisor"), \
