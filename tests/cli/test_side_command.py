...
@pytest.fixture
def cli_instance(session_db, tmp_path):
    cli = MagicMock()
...
class TestSideCommandCLI:
    ...
    def test_side_without_title_leaves_session_eligible_for_auto_title(self, cli_instance, session_db):
        ...
        side_id = cli_instance.session_id
        meta = session_db.get_session_title_metadata(side_id)
        assert meta["title"] is None
        assert meta["title_source"] is None

        active = session_db.get_active_side_session(source="cli")
        assert active["side_session_id"] == side_id
        assert active["title"] is None

    def test_side_with_title_marks_title_as_manual(self, cli_instance, session_db):
        from cli import HermesCLI

        HermesCLI._handle_side_command(cli_instance, "/side ncore quick question")

        meta = session_db.get_session_title_metadata(cli_instance.session_id)
        assert meta["title"] == "ncore quick question"
    ...
    def test_back_without_active_side_session_is_noop(self, cli_instance, session_db):
        from cli import HermesCLI
