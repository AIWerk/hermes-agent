...
def test_gateway_side_back_restores_parked_main_session(tmp_path, monkeypatch):
    ...
    assert all(message.get("_db_persisted") is True for message in sess["history"])
    assert db.get_active_side_session(source="tui-test") is None


def test_side_start_persists_bound_actor_on_child_session(tmp_path, monkeypatch):
    import hermes_state
    ...
    monkeypatch.setattr(
        ...
        lambda sid, session: {"model": "test-model"},
    )
    monkeypatch.setattr(server, "_notify_session_boundary", lambda *a, **k: None)
    monkeypatch.setattr(server, "_emit", lambda *a, **k: None)
    ...
    sess["cui_actor_context"] = dict(actor)
    server._sessions[sid] = sess
    token = server.bind_cui_actor_context(actor)
    try:
        response = server._methods["session.side.start"](
            "r-actor",
            {"session_id": sid, "title": "Actor side"},
        )
    finally:
    ...
    row = db.get_session(response["result"]["side_session_id"])
    assert row is not None
    config = json.loads(row["model_config"])
    assert config["_cui_actor_context"] == actor
