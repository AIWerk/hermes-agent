...
def test_approval_pending_replays_unresolved_requests(server, monkeypatch):
    ...
    assert response["result"]["approvals"][0]["request_id"] == "req-1"
    assert secret not in repr(response["result"])
    assert secret in pending[0]["command"]
    assert secret in pending[0]["description"]
...
def test_session_resume_returns_hydrated_messages(server, monkeypatch):
    ...
    assert resp["result"]["messages"] == [
        ...
        {"role": "tool", "name": "tool", "context": "tool"},
    ]
