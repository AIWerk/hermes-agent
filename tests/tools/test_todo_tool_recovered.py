...
class TestTodoStoreBounds:
    ...
    def test_replace_preserves_loaded_open_unmanaged_markdown_item(self, tmp_path):
        todo_path = tmp_path / "TODO.md"
        todo_path.write_text("- [ ] Customer task\n")
        store = TodoStore(todo_path)

        result = store.write(
            [{"id": "agent-1", "content": "Agent task", "status": "pending"}],
            merge=False,
        )

        assert {item["content"] for item in result} == {"Customer task", "Agent task"}
