...
@pytest.mark.skipif(sys.platform == "win32", reason="POSIX env -i contract")
def test_shell_runner_disables_lazy_installs_before_collection(tmp_path: Path) -> None:
    """The canonical runner seals lazy installs before any test-module import."""
    repo_root = Path(__file__).resolve().parent.parent
    requested_tmp = tmp_path / "caller-tmp"
    requested_tmp.mkdir()
    probe = tmp_path / "test_lazy_install_seal_probe.py"
    probe.write_text(
        "import os\n\n"
        "assert os.environ.get('HERMES_DISABLE_LAZY_INSTALLS') == '1'\n"
        "assert 'HERMES_LAZY_INSTALL_TARGET' not in os.environ\n\n"
        "def test_runner_sealed_before_collection():\n"
        "    assert True\n",
        encoding="utf-8",
    )
    env = os.environ.copy()
    ...
    env["HERMES_LAZY_INSTALL_TARGET"] = str(tmp_path / "forbidden-target")
