...
def test_acp_todo_plan_sanitizes_customer_visible_content():
    secret = "abcdefghijklmnopqrstuvwxyz123456"
    update = events._build_plan_update_from_todo_result(
        '{"todos":[{"id":"task-1","content":"Authorization: Bearer '
        + secret
        + '","status":"pending"}]}'
    )
    ...
    assert secret not in repr(update)
    assert "Authorization" in repr(update)


def test_acp_history_message_and_thought_replay_sanitize_stored_credentials():
    secret = "abcdefghijklmnopqrstuvwxyz123456"
    message = HermesACPAgent._history_message_update(
        role="assistant",
        text=f"Authorization: Bearer {secret}",
    )
    thought = HermesACPAgent._history_thought_update(
