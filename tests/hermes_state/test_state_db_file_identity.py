...
def test_deferred_fts_retry_detects_replacement_before_rebuild(tmp_path):
    live = tmp_path / "state.db"
    other = tmp_path / "other.db"
    db = _make_db(live, "live-sess", "original")
    _require_identity(db)
...
def test_deferred_fts_retry_revalidates_after_admission(tmp_path, monkeypatch):
    ...
    @contextmanager
    def _replace_during_admission(*_args, **_kwargs):
        os.replace(other, live)
        yield True

    monkeypatch.setattr(hermes_state_schema, "fts_rebuild_admission", _replace_during_admission)
...
def test_same_file_fts_corruption_still_fails_open(tmp_path):
    ...
    # Exercise the durable fail-open path after the retained one-shot live
    # self-heal budget has already been consumed.
    db._fts_runtime_rebuild_attempted = True
