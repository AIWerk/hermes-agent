...
In a gateway process shared by multiple clients (all keyed by the same
process-wide source), an unscoped pop lets one client's /back pop another
client's parked session and load its parent history.
"""
...
def test_pop_side_session_returns_none_for_unknown_side_session(tmp_path):
    ...
    # A client whose current session never parked anything pops nothing.
