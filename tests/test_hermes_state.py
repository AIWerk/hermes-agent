...
class TestFTS5Search:
    ...
    def test_search_projection_skips_context_enrichment_queries(self, db):
        ...
        with mock.patch.object(db, "_read_ctx", wraps=db._read_ctx) as read_ctx:
            ...
            before_default = read_ctx.call_count
            default = db.search_messages("projectionneedle")
            assert len(default) == 1
            assert default[0]["context"]
            default_reads = read_ctx.call_count - before_default
...
class TestFTSExternalContentMigration:
    ...
    def test_optimize_refuses_to_stamp_when_fts_integrity_verification_fails(
        self, tmp_path, monkeypatch
    ):
        db_path = tmp_path / "v22.db"
        self._build_v22_db(db_path)
        ...
        try:
            monkeypatch.setattr(
                db,
                "_verify_fts_repair",
                lambda _conn: (False, "injected external-content mismatch"),
            )
...
class TestSessionArchive:
    ...
    def test_hidden_excluded_from_default_list_and_paired_count(self, db):
        self._seed(db, "visible")
        self._seed(db, "hidden")
        db.set_session_hidden("hidden", True)

        assert [row["id"] for row in db.list_sessions_rich()] == ["visible"]
        assert db.session_count() == 1
        assert db.session_count(include_hidden=True) == 2
