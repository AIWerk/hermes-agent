...
class TestCompactRows:
    ...
    return only the explicit public metadata projection."""
    ...
    def test_compact_projection_tracks_schema(self, db):
        """Every schema column is classified; public rows expose only public fields."""
        self._create(db, "s1")
        live_cols = {
            row[1] for row in db._conn.execute("PRAGMA table_info(sessions)")
        }
        row = db.list_sessions_rich(compact_rows=True)[0]
        assert set(db._SESSION_COLUMN_VISIBILITY) == live_cols
        public = {
            name
            for name, visibility in db._SESSION_COLUMN_VISIBILITY.items()
            if visibility == "public"
        }
        assert public <= set(row)
        assert not ({"session_key", "system_prompt_hash", "git_repo_root"} & set(row))
        assert "system_prompt" not in row
    ...
    def test_batch_compact_rows_omits_internal_git_metadata(self, db):
        ...
        system_prompt blob; public git_branch remains but repo root is internal."""
        self._create(db, "s1", system_prompt="should be gone")
        ...
        assert "git_repo_root" not in row
    def test_compression_tip_projection_threads_compact_rows(self, db):
        ...
        system_prompt and internal git metadata (guards the call site at the
        ...
        assert "git_repo_root" not in tip
