...
def test_foreign_holder_scan_returns_empty_for_known_zero_openers(monkeypatch, tmp_path):
    db_path = tmp_path / "state.db"
    _make_db(db_path, "live")

    def _listdir(path):
        if path == "/proc":
            return [str(os.getpid()), "424242"]
        if path == "/proc/424242/fd":
            return []
        return []

    monkeypatch.setattr(backup_mod.os, "listdir", _listdir)
    monkeypatch.setattr(backup_mod.sys, "platform", "linux")

    assert backup_mod._foreign_db_holder_pids(db_path) == []


def test_foreign_holder_scan_treats_unreadable_fd_dir_as_unknown(monkeypatch, tmp_path):
    db_path = tmp_path / "state.db"
    _make_db(db_path, "live")
    own_pid = os.getpid()
    ...
    def _listdir(path):
        if path == "/proc":
            return [str(own_pid), "424242"]
        if path == "/proc/424242/fd":
            raise PermissionError("root-owned fd directory")
        return []

    monkeypatch.setattr(backup_mod.os, "listdir", _listdir)
    monkeypatch.setattr(backup_mod.sys, "platform", "linux")

    assert backup_mod._foreign_db_holder_pids(db_path) is None


def test_foreign_holder_scan_treats_unreadable_fd_target_as_unknown(monkeypatch, tmp_path):
    db_path = tmp_path / "state.db"
    _make_db(db_path, "live")
    own_pid = os.getpid()
    ...
    monkeypatch.setattr(
        backup_mod.os,
        "readlink",
        lambda _path: (_ for _ in ()).throw(PermissionError("root-owned fd")),
    )
    monkeypatch.setattr(backup_mod.sys, "platform", "linux")
