...
class TestPatchRetryOnVulnerableCandidate:
    ...
    def test_retry_is_bounded_by_max_retries_constant(self, tmp_path, monkeypatch):
        ...
        # After the bounded 3.11 patch ladder, upstream falls back to a fixed
        # 3.12 runtime rather than returning failure.
        assert result is not None
        assert result[-1].python_version == (3, 12)
        # The same-minor ladder remains bounded before the cross-minor fallback.
