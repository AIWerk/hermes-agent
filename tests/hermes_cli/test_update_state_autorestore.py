...
def test_restore_helper_serves_snapshot_rows_over_a_hot_wal(
    live_db_with_hot_wal, snapshot_db, monkeypatch
):
...
def test_restore_helper_refuses_unknown_opener_state_before_copy(
    ...
    _make_valid_db(state_path, 1)
    _make_valid_db(snapshot, 2)
    for suffix in ("-wal", "-shm", "-journal"):
        _sidecar(state_path, suffix).write_bytes(b"stale")

    cleared = []
    copied = []
    monkeypatch.setattr(backup_mod, "_foreign_db_holder_pids", lambda _path: None)
    monkeypatch.setattr(update_cmd_maint, "_clear_stale_sqlite_sidecars", lambda path: cleared.append(path))
    monkeypatch.setattr(update_cmd_maint.shutil, "copy2", lambda src, dst: copied.append((src, dst)))

    assert _restore_state_db_from_snapshot(state_path, snapshot) is False

    assert "Auto-restore refused" in capsys.readouterr().out
    assert cleared == []
    assert copied == []
    for suffix in ("-wal", "-shm", "-journal"):
        assert _sidecar(state_path, suffix).exists()
...
def test_restore_helper_propagates_copy_errors(tmp_path, monkeypatch):
...
def test_post_update_guard_covers_sibling_profiles(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("hermes_cli.backup._foreign_db_holder_pids", lambda _path: [])
