...
def test_recovery_fts_verifier_detects_external_content_mismatch():
    ...
    try:
        ...
        conn.execute("INSERT INTO messages(id, content) VALUES (1, 'missing from index')")
        verification = {"errors": []}

        session_recovery._verify_fts_indexes(conn, verification)

        assert verification["fts_checks"]["messages_fts"] != "ok"
