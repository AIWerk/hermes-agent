...
from tui_gateway.turn_marker import clear_turn_marker, record_turn_start

from .method_ctx import HandlerRegistry, bind_module

if TYPE_CHECKING:
    from tui_gateway.server import (
        _GOAL_COMPRESSION_RECOVERY_ATTEMPTS, _GOAL_COMPRESSION_RECOVERY_LIMIT,
        _CRASH_LOG, _active_image_routing_identity, _append_inflight_delta,
        _apply_model_switch, _apply_pending_model_switch, _arm_full_duplex_listener,
        _build_image_ref_message, _build_persist_user_message, _clear_inflight_turn,
        _clear_session_context, _context_cwd_is_launch_artifact,
        _current_runtime_session_record, _drain_queued_prompt, _emit,
        _emit_settled_session_info, _emit_terminal_turn_error, _enqueue_prompt,
        _ensure_active_session_slot,
        _fail_inflight_turn, _get_usage, _hud_surface_note, _inflight_text,
        _is_pivot_marker, _load_cfg, _load_interim_assistant_messages,
        _notif_handle_ready, _pending_reaction_notes, _prepend_note,
        _publish_session_control_snapshot, _register_session_cwd,
        ...
        _sync_agent_model_with_config, _sync_bot_capabilities,
        _sync_session_key_after_compress, _turn_failure_detail, _tts_stream_begin,
        _voice_cfg_dict, _voice_mode_enabled, _voice_tts_enabled, _wire_callbacks,
        logger, make_stream_renderer, render_message,
    )
...
def _prepare_turn_input(sid: str, session: dict, st: _TurnRun, text: Any, images: list[str]):
    ...
    if profile_home:
        ...
        scopes.secret = set_secret_scope(_build_profile_secret_scope(Path(profile_home)))
