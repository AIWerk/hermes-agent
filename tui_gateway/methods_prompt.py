...
def _(rid, params: dict) -> dict:
    ...
    if survivor_user_row_ids is not None:
        response["survivor_user_row_ids"] = survivor_user_row_ids
    if auto_reset:
    ...
    def run():
        ...
        try:
            ...
            _emit(
                ...
                {
                    "task_id": task_id,
                    "text": sanitize_tool_display_text(
            ...
            _emit(
                "background.complete",
                parent,
                {"task_id": task_id, "text": sanitize_tool_display_text(f"error: {e}")},
            )
        finally:
    ...
    def run():
        ...
        try:
            ...
            _emit(
                ...
                parent,
                {"task_id": task_id, "text": sanitize_tool_display_text(text)},
            )
        except Exception as e:
            from agent.tool_argument_projection import sanitize_tool_display_text
            ...
            _emit(
                ...
                {
                    "task_id": task_id,
                    "text": sanitize_tool_display_text(f"error: {e}"),
                },
            )
        finally:
            try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        pending = list_gateway_approvals(session["session_key"])
        return _ok(
            rid,
            {"approvals": [_approval_request_payload(item) for item in pending]},
        )
    except Exception as e:
