...
def _reconcile_client_ordinal(
    ...
    *,
    ...
    """Validate an optional client ordinal against a durable truncation target."""
    if client_ordinal is not None and client_ordinal != resolved_ordinal and not (
        prefix_user_count > 0 and client_ordinal == resolved_ordinal + prefix_user_count
    ):
        logger.warning(
            "prompt.submit: REFUSED truncation due to ordinal mismatch for session %s "
            "(ordinal=%d, %s_ordinal=%d, %s=%s, prefix_user_count=%d). "
        ...
        return None, _err(
            ...
            4030,
            f"truncate_before_user_ordinal ({client_ordinal}) does not match "
            f"{param_name} target turn ({resolved_ordinal})",
        )
    return resolved_ordinal, None
