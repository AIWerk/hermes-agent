...
if TYPE_CHECKING:
    from tui_gateway.server import (
        _DASHBOARD_UPLOAD_ROOT, _IMAGE_ATTACHMENT_EXTENSIONS, _err, _hermes_home, _session_cwd,
    )
...
def _session_upload_component(session_id: Any) -> str:
    ...
    safe = _re.sub(r"[^A-Za-z0-9._-]+", "-", str(session_id or "")).strip(".-_")
...
_INBOUND_SHARED_URI_RE = _re.compile(r"\bshared://[^\s)\]>'\"]+", _re.IGNORECASE)
_INBOUND_IMAGE_HINT_RE = _re.compile(r"vision_analyze\s+using\s+image_url:\s+([^\]\n]+)", _re.IGNORECASE)
...
def _shared_uri_relative_path(uri: str) -> str | None:
    ...
    rel = _urllib_parse.unquote(rel).replace("\\", "/")
...
def _shared_uri_prompt_attachments(text: Any, session_id: Any) -> list[dict[str, Any]]:
    ...
    for match in _INBOUND_SHARED_URI_RE.finditer(text):
        ...
        item = {
            "name": Path(rel).name,
            "reference_uri": f"shared://{_urllib_parse.quote(rel, safe='/')}",
            "open_url": "/api/assistant/shared-folder/open?path="
            f"{_urllib_parse.quote(rel, safe='')}",
        }
        try:
...
def _inbound_image_attachment_payloads(text: Any) -> list[dict[str, Any]]:
    ...
    for match in _INBOUND_IMAGE_HINT_RE.finditer(text):
        ...
        try:
            ...
            size = 0
        open_url = "/api/assistant/artifacts/open?path=" f"{_urllib_parse.quote(str(path), safe='')}"
        attachments.append(
            {
                "name": path.name,
                "path": str(path),
