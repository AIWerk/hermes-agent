...
if TYPE_CHECKING:
    from tui_gateway.server import (
        _TURN_SETTLE_BEFORE_CLOSE_SECONDS, _WS_ORPHAN_ACTIVITY_STALE_S,
        _WS_ORPHAN_INTERRUPT_REAP_MAX_POLLS, _WS_ORPHAN_INTERRUPT_REAP_POLL_S,
        _WS_ORPHAN_REAP_GRACE_S, DESKTOP_BACKEND_CONTRACT, _broadcast_global_event,
        _clear_inflight_turn, _clear_pending, _current_profile_name, _detached_ws_transport,
        _display_session_cwd, _emit, _err, _get_compute_host_supervisor, _get_db, _load_cfg,
        _new_session_key, _register_session_cwd, _resolve_agent_platform, _session_db,
        _session_resume_lock, _session_source, _session_uses_compute_host, _sessions,
        _sessions_lock, _transport_is_dead, logger,
    )
...
def _cui_session_reset_reason(db, session_key: str) -> str | None:
    ...
    now = time.time()
    if policy["mode"] in {"idle", "both"} and now > last_active + policy["idle_minutes"] * 60:
        return "idle"
    if policy["mode"] in {"daily", "both"}:
        boundary = datetime.fromtimestamp(now).replace(hour=policy["at_hour"], minute=0, second=0, microsecond=0).timestamp()
...
def _rebind_live_transport(sid: str, session: dict, transport: Transport) -> None:
    ...
    try:
        ...
        session.setdefault("viewers", {})[id(transport)] = time.time()
    # See #83716.
