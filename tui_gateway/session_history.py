...
if TYPE_CHECKING:
    from tui_gateway.server import (
        _inbound_image_attachment_payloads, _outbound_attachment_payloads_and_text,
        _sanitize_outbound_customer_value, _tool_ctx, logger,
    )
...
def _history_to_messages(history: list[dict]) -> list[dict]:
    ...
    for m in history:
        ...
        if role == "assistant":
            ...
            msg.update(
                (key, _sanitize_outbound_customer_value(m[key]))
        ...
        if m.get("display_metadata"):
            msg["display_metadata"] = _sanitize_outbound_customer_value(m["display_metadata"])
