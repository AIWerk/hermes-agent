...
# AIWerk retained actor, session, and customer-projection contracts bind through this facade.
import uuid
...
def _dashboard_user_for_cui_actor(actor: dict[str, str]) -> dict:
    ...
    for user in users:
        if not isinstance(user, dict):
            continue
        user_actor_id = str(user.get("actor_id") or "").strip()
        user_tenant_id = str(user.get("tenant_id") or "").strip()
        user_provider = str(user.get("provider") or "").strip()
        if not user_actor_id or not user_tenant_id:
            continue
        if user_actor_id != actor_id or user_tenant_id != tenant_id:
            continue
        if (provider or user_provider) and (not provider or not user_provider or provider != user_provider):
            continue
        return user
    return {}
...
_PERSISTED_SESSION_SCAN_PAGE_SIZE = 100
...
def _db_for_profile(profile: str | None = None):
    """Return ``(db, owns_handle)`` for launch or profile-scoped session storage."""
...
def _emit(event: str, sid: str, payload: dict | None = None):
    if event == "message.start":
        with _OUTBOUND_DELTA_PROJECTION_LOCK:
            for key in [key for key in _OUTBOUND_DELTA_PROJECTION_BUFFERS if key[1] == sid]:
    ...
    if event in {"message.delta", "reasoning.delta"} and isinstance(payload, dict):
        ...
        with _OUTBOUND_DELTA_PROJECTION_LOCK:
            state = _OUTBOUND_DELTA_PROJECTION_BUFFERS.get(buffer_key, ("", False))
            emitted, next_state = _project_outbound_stream_fragment(state, incoming)
            if next_state == ("", False):
                _OUTBOUND_DELTA_PROJECTION_BUFFERS.pop(buffer_key, None)
            else:
                _OUTBOUND_DELTA_PROJECTION_BUFFERS[buffer_key] = next_state
        if not emitted:
            return
        payload = dict(payload)
        payload["text"] = emitted
        payload.pop("rendered", None)
    if event == "message.complete":
        with _OUTBOUND_DELTA_PROJECTION_LOCK:
            for key in [key for key in _OUTBOUND_DELTA_PROJECTION_BUFFERS if key[1] == sid]:
                buffered, _state = _OUTBOUND_DELTA_PROJECTION_BUFFERS.pop(key, ("", False))
                if buffered:
                    write_json(_event_frame(
...
def _stamp_cui_actor_context(model_config: dict, actor_context: dict | None) -> None:
    ...
    model_config["_cui_visibility_scope"] = "admin" if role in _CUI_ADMIN_ROLES else "customer"
...
class _CuiActorScopedSessionDB:
    ...
    def _visible_rows(self, kwargs: dict) -> list[dict]:
        ...
        while True:
            ...
            rows.extend(
                row for row in page
                if _session_list_row_visible_to_cui_actor(self._db, row, self._actor)
            )
            if len(page) < 1000:
                return rows
            offset += len(page)
    ...
    def adopt_session_lineage_from(self, donor_db, session_id):
        ...
        raw_donor = donor_db._db if isinstance(donor_db, _CuiActorScopedSessionDB) else donor_db
...
def _session_visible_to_cui_actor(row: dict, actor_context: dict | None) -> bool:
    return _row_visible_to_cui_actor(row, actor_context)


def _session_list_row_visible_to_cui_actor(db, row: dict, actor_context: dict | None) -> bool:
    """Authorize both ownership-bearing root metadata and a projected compression tip."""
    if not _row_visible_to_cui_actor(row, actor_context):
        return False
    ...
    if not projected_id or not lineage_root_id or projected_id == lineage_root_id:
        return True
    return _row_visible_to_cui_actor(db.get_session(projected_id), actor_context)


def _project_persisted_session_row_public(row: dict) -> dict:
    allowed = {"id", "source", "title", "preview", "started_at", "last_active", "ended_at", "message_count"}
...
def _iter_visible_persisted_session_rows(
    ...
    *,
    ...
    public_columns = {name for name, visibility in visibility_map.items() if visibility == "public"}
    ...
    while True:
        ...
        for row in page:
            source = str(row.get("source") or "").strip().lower()
            if source not in deny_sources and _session_list_row_visible_to_cui_actor(
                db, row, actor_context
            ):
                yield {key: value for key, value in row.items() if key in public_columns}
        page_count = len(page)
...
def _live_session_visible_to_cui_actor(session_id: str, actor_context: dict | None) -> bool:
    ...
    if isinstance(session_id, dict):
        ...
        with _sessions_lock:
            session = _sessions.get(session_id)
    return _live_record_visible_to_cui_actor(session, actor_context)


def _live_record_visible_to_cui_actor(session: dict | None, actor: dict | None = None) -> bool:
    ...
    try:
        ...
        def run():
            try:
                ...
                _store_created_session_actor(req, resp)
            except Exception as exc:
        ...
        _clear_cui_actor_env(actor_token)
...
def _find_live_session(target: str) -> tuple[str, dict] | None:
    ...
    for sid, session in snapshot:
        if session.get("_finalized") or not _live_record_visible_to_cui_actor(session):
            continue
...
from . import (  # noqa: E402
    ...
    outbound_artifacts as _outbound_artifacts,
...
for _m in (
    ...
    _outbound_artifacts, _prompt_attachments, _session_history, _agent_callbacks, _session_auto_continue,
