    ...
    try:
        ...
        if title_lookup:
            ...
            try:
                ...
                tip = row["id"]
            if tip != row["id"]:
                tip_row = _visible_persisted_session_row(db, tip, cui_actor)
                if tip_row is None:
                    return _ok(rid, {"sessions": []})
            else:
                tip_row = row
...
def _find_live_unpersisted(needle: str, home) -> str:
    ...
    return next((
        ...
        and _live_record_visible_to_cui_actor(record)
...
def _resume_live_unpersisted(ctx: _Resume, live_sid: str, live: dict) -> dict:
    ...
    return _ok(ctx.rid, _attach_todo_state({
        "session_id": live_sid, "session_key": str(live.get("session_key") or ""),
...
def _resume_follow_tip(ctx: _Resume) -> None:
    ...
    if tip and tip != ctx.target:
        ctx.target = tip
        ctx.found = ctx.db.get_session(tip)
    ...
    try:
        if callable(safety_check := getattr(ctx.db, "assert_resume_safe", None)):
            try:
                ...
                if not tip_only or "tip_only" not in str(exc):
                    raise
...
def _(rid, params: dict) -> dict:
    ...
    rows = [
        ...
        if not session.get("_finalized") and _live_record_visible_to_cui_actor(session)
    ]
...
def _reset_session_agent(sid: str, session: dict) -> dict:
    ...
    tokens = _set_session_context(session["session_key"])
    try:
        for key in ("model_override", "create_reasoning_override", "create_service_tier_override",
...
def _switch_live_session(sid: str, session: dict, target_session_id: str, history: list, *, reason: str) -> dict:
    ...
    with contextlib.suppress(Exception):
        ...
        register_gateway_notify(target_session_id, lambda data: _emit_approval_request(sid, data))
    with session["history_lock"]:
    ...
    _emit("session.switched", sid, {"session_id": target_session_id, "reason": reason, "info": info})
    return info
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        _stamp_cui_actor_context(model_config, current_cui_actor_context())
        if agent:
            model_config.update(
                {
        ...
        db.push_side_session(source=source, parent_session_id=parent_session_id, side_session_id=side_session_id, title=title)
