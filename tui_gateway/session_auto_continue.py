...
from tui_gateway.turn_marker import clear_turn_marker, read_turn_marker
...
if TYPE_CHECKING:
    from tui_gateway.server import (
        _AUTO_CONTINUE_NOTE_PREFIX, _clear_inflight_turn, _coerce_int_config_value,
        _coerce_message_text, _emit, _ensure_active_session_slot, _fail_inflight_turn,
        _get_compute_host_supervisor, _get_usage, _hermes_home, _inflight_text,
        _is_text_only_busy_payload, _load_busy_input_mode, _load_cfg,
        _notif_log_failure, _notif_release_turn, _ok, _record_inflight_correction,
        _run_prompt_submit, _session_uses_compute_host, _start_agent_build,
        _sanitize_outbound_customer_value, _submit_prompt_to_compute_host,
        _wait_agent, logger, render_message,
    )
...
def _inflight_snapshot(session: dict) -> dict | None:
    ...
    if not (user or assistant or streaming or error):
        return None
    snapshot = {
        "assistant": _sanitize_outbound_customer_value(assistant),
        "streaming": streaming,
        "user": user,
    }
    ...
    if error:
        ...
        snapshot.update(
            error=_sanitize_outbound_customer_value(error),
            status=str(turn.get("status") or "error"),
...
def _emit_terminal_turn_error(
    ...
    with session["history_lock"]:
        ...
        message = str(_sanitize_outbound_customer_value(turn.get("error") or "turn failed"))
        partial = str(_sanitize_outbound_customer_value(turn.get("assistant") or ""))
