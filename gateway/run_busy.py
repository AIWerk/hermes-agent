...
class GatewayBusySessionMixin:
    ...
    def _queue_or_replace_pending_event(self, session_key: str, event: MessageEvent) -> None:
        ...
        if self._queue_depth(session_key, adapter=adapter) >= self._BUSY_QUEUE_MAX_PENDING:
            ...
            return
        media_merge_candidate = same_security_context and (
        ...
        if (
            media_merge_candidate
            and getattr(existing, "media_urls", None)
            and len(existing.media_urls) >= self._BUSY_QUEUE_MAX_PENDING
        ):
            logger.warning(
                "Dropping busy-mode media follow-up for session %s — merged media at cap (%d).",
                session_key, self._BUSY_QUEUE_MAX_PENDING,
            )
            return
        if media_merge_candidate:
            before = pending_slot.get(session_key)
            merge_pending_message_event(
                adapter._pending_messages, session_key, event,
                merge_text=event.message_type == MessageType.TEXT,
            )
            if pending_slot.get(session_key) is before:
                # Preserve photo-burst / media-merge semantics for the head slot only when
                # identity checks inside merge_pending_message_event accepted the merge.
                event._gateway_accepted = True
                return
            pending_slot[session_key] = before
    ...
    async def _resolve_busy_steer_or_redirect(
        ...
        if effective_mode == "steer":
            ...
            and (
                ...
                or getattr(running_agent, "_executing_tools", False) is True
            )
    ...
    def _try_agent_verb(
        ...
        try:
            ...
            if verb in {"redirect", "steer"} and hasattr(running_agent, "_open_turn_correction_admission"):
                running_agent._open_turn_correction_admission()
