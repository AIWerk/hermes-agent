                ...
                if (self._should_auto_tts_for_chat(event.source.chat_id)
                    ...
                    try:
                        ...
                        if check_tts_requirements():
                            ...
                            # AIWerk policy: Telegram's native voice bubbles
                            # auto-play as a playlist in some clients. Automatic
                            # replies therefore use sendAudio/MP3 there, while
                            # the other Opus platforms keep native Ogg/Opus.
                            if _platform_name(self.platform) == "telegram":
                                _tts_requested_path = os.path.join(
                                    tempfile.gettempdir(),
                                    "hermes_voice",
                                    f"auto_tts_{uuid.uuid4().hex[:12]}.mp3",
                                )
                                os.makedirs(os.path.dirname(_tts_requested_path), exist_ok=True)
                ...
                for _tts_index, _tts_path in enumerate(_tts_paths):
                    try:
                        ...
                        _tts_caption_delivered = bool(
                            ...
                            or getattr(tts_result, "success", False)
                        )
                    finally:
                        try:
                            os.remove(_tts_path)
