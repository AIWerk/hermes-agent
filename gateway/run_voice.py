...
with suppress(Exception):
    import tools.tts_tool as _tts_tool_module
    from tools.tts_text_normalize import _strip_markdown_for_tts as _tts_strip_markdown

    if not hasattr(_tts_tool_module, "_strip_markdown_for_tts"):
        _tts_tool_module._strip_markdown_for_tts = _tts_strip_markdown
...
class GatewayVoiceMixin:
    ...
    def _should_suppress_text_streaming_for_voice_reply(
        self, source: SessionSource, message_type: MessageType
    ) -> bool:
        """True when an explicit voice-only voice turn should withhold the text stream."""
        return (
            message_type == MessageType.VOICE
            and self._voice_mode.get(self._voice_key_for_source(source)) == "voice_only"
        )
